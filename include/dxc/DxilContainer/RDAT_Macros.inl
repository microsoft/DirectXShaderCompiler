...
#undef RDAT_ARRAY_VALUE
  ...
  #define RDAT_STRUCT_DERIVED(type, base) RDAT_STRUCT(type) \
    template<> struct RecordBaseTrait<type> { \
      typedef RecordBaseTrait<base>::FirstBaseType FirstBaseType; \
      typedef base BaseType; \
    }; \
    template<> struct RecordDerivedTrait<base> { \
      typedef RecordDerivedTrait<type>::LastDerivedType LastDerivedType; \
      typedef type DerivedType; \
    };
...
#undef RDAT_ENUM_END
  ...
  #define RDAT_DXIL_ENUM_ASSERT_VALUE(scoped_name, value) \
    static_assert((unsigned)scoped_name == value, "otherwise, RDAT_DXIL_ENUM definition needs updating");
