...
; The vector pointed to by the groupshared memory operand of the LinAlgMatrix
; memory operations must have a legal component type. i8 is not one.

; CHECK: error: DXIL intrinsic overload must be valid.
; CHECK-NEXT: note: at {{.*}}@dx.op.linAlgMatrixLoadFromMemory.mC9M4N4U1S1.v4i8
; CHECK: error: DXIL intrinsic overload must be valid.
; CHECK-NEXT: note: at {{.*}}@dx.op.linAlgMatrixStoreToMemory.mC9M4N4U1S1.v4i8
; CHECK: error: DXIL intrinsic overload must be valid.
; CHECK-NEXT: note: at {{.*}}@dx.op.linAlgMatrixAccumulateToMemory.mC9M4N4U2S1.v4i8
...
define void @main() {
  ...
  %2 = call %dx.types.LinAlgMatrixC9M4N4U2S1 @dx.op.linAlgFillMatrix.mC9M4N4U2S1.i32(i32 -2147483636, i1 true, i32 0)  ; LinAlgFillMatrix(isInputSigned,value)
...
declare %dx.types.LinAlgMatrixC9M4N4U1S1 @dx.op.linAlgMatrixLoadFromMemory.mC9M4N4U1S1.v4i8(i32, <4 x i8> addrspace(3)*, i32, i32, i32) #0
