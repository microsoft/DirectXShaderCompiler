...
void CSMain()
{
  // CHECK: call %"struct.MyStructA<float>"* @"dx.hl.op.ro.%\22struct.MyStructA<float>\22* (i32, %dx.types.Handle, <1 x i32>)"(i32 {{[0-9]+}}, %dx.types.Handle %{{[0-9]+}}, <1 x i32> zeroinitializer)
  MyStructA<float> a = g_bab.Load<MyStructA<float> >(0);
  result[0] = a.m_0;

  // CHECK: call %struct.MyStructB* @"dx.hl.op.ro.%struct.MyStructB* (i32, %dx.types.Handle, <1 x i32>)"(i32 {{[0-9]+}}, %dx.types.Handle %{{[0-9]+}}, <1 x i32> <i32 1>)
