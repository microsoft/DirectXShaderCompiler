...
float CreateLiveVal( int2 idx )
{
  float ret = outputBuffer[idx].x;
  return ret;
}
...
void ClosestHit( inout Payload payload, in Attribs attribs )
{
  ...
  float liveFloat = CreateLiveVal(idxA);  // Create a live value within the function; LVA should trace to the original value inside.
