...
// CHECK: [[cube_level:%[a-zA-Z0-9_]+]] = OpImageSampleExplicitLod %v4float [[cube_load]] {{%[0-9]+}} Lod %float_0_5
    float4 val7 = texCube.SampleLevel(float3(0.5, 0.25, 0), 0.5f);

// CHECK: [[cube_arr_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_sampled_image_array]] %texCubeArray
// CHECK: [[cube_arr_level:%[a-zA-Z0-9_]+]] = OpImageSampleExplicitLod %v4float [[cube_arr_load]] [[v4fc]] Lod %float_0_5
    float4 val8 = texCubeArray.SampleLevel(float4(0.5, 0.25, 0, 0), 0.5f);
