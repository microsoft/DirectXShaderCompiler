...
DxilMDHelper::EmitDxilFunctionProps(const hlsl::DxilFunctionProps *props,
  ...
  case DXIL::ShaderKind::Mesh:
    ...
    MDVals[valIdx++] =
        Uint32ToConstMD(props->ShaderProps.MS.payloadSizeInBytes);
    break;
  case DXIL::ShaderKind::Amplification:
    ...
    MDVals[valIdx++] =
        Uint32ToConstMD(props->ShaderProps.AS.payloadSizeInBytes);
    break;
  default:
    break;
  }
...
void DxilMDHelper::EmitDxilCounters(const DxilCounters &counters) {
  ...
  vector<Metadata *> MDVals;
  // <py::lines('OPCODE-COUNTERS')>['AddCounterIfNonZero(counters.%s, "%s",
  // MDVals);' % (c,c) for c in hctdb_instrhelp.get_counters()]</py>
