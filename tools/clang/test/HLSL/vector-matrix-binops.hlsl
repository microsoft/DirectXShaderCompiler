// RUN: %clang_cc1 -fsyntax-only -ffreestanding -verify %
...
void main() {
    ...
    // vector truncation
    {
      float2 res1 = v2 * v4; // expected-warning {{implicit truncation of vector type}} expected-warning {{implicit truncation of vector type}} 
      float2 res2 = v4 - v3; // expected-warning {{implicit truncation of vector type}} expected-warning {{implicit truncation of vector type}} 
    }
    ...
    // matrix mismatched dimensions
    {
      ...
      float3x2 res1 = m23 - m32; // expected-error {{cannot convert from 'float3x2' to 'float2x3'}}
