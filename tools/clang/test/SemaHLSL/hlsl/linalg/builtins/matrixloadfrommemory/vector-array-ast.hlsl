...
// CHECK: FunctionDecl {{.*}} implicit used __builtin_LinAlg_MatrixLoadFromMemory 'void (__builtin_LinAlgMatrix {{.*}}, vector<float, 4> __attribute__((address_space(3))) (&)[64], unsigned int, unsigned int, unsigned int)' extern
// CHECK-NEXT: ParmVarDecl {{.*}} ret '__builtin_LinAlgMatrix {{.*}}'
// CHECK-NEXT: ParmVarDecl {{.*}} memory 'vector<float, 4> __attribute__((address_space(3))) (&)[64]'
...
void main() {
  ...
  dx::__builtin_LinAlg_MatrixLoadFromMemory(mat, SharedArr, 0, 0, 0);
}
