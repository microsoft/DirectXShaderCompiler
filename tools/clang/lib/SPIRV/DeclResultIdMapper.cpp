...
void DeclResultIdMapper::createGlobalsCBuffer(const VarDecl *var) {
  ...
  if (auto *ptrType = dyn_cast<SpirvPointerType>(globals->getResultType())) {
    structType = dyn_cast<StructType>(ptrType->getPointeeType());
  } else {
    const auto *hybridStruct =
        dyn_cast<HybridStructType>(globals->getResultType());
    structType = spvBuilder.lowerHybridStructType(hybridStruct);
    // Store the concrete type back on the variable so later passes reuse
    globals->setResultType(structType);
  }
  assert(structType);
  spvContext.registerStructDeclForSpirvType(structType,
