...
// CHECK: [[cube_grad:%[a-zA-Z0-9_]+]] = OpImageSampleExplicitLod %v4float [[cube_load]] {{%[0-9]+}} Grad [[v3f_1]] [[v3f_2]]
    float4 val8 = texCube.SampleGrad(float3(0.5, 0.25, 0), float3(1, 1, 1), float3(2, 2, 2));

// CHECK: [[cube_arr_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_sampled_image_array]] %texCubeArray
// CHECK: [[cube_arr_grad:%[a-zA-Z0-9_]+]] = OpImageSampleExplicitLod %v4float [[cube_arr_load]] [[v4fc]] Grad [[v3f_1]] [[v3f_2]]
    float4 val9 = texCubeArray.SampleGrad(float4(0.5, 0.25, 0, 0), float3(1, 1, 1), float3(2, 2, 2));
