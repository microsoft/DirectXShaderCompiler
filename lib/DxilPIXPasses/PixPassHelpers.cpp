...
namespace PIXPassHelpers
{
    ...
    static unsigned int GetNextRegisterIdForClass(hlsl::DxilModule& DM, DXIL::ResourceClass resourceClass) {
    ...
    llvm::CallInst *
      ...
      unsigned int resourceMetaDataId = GetNextRegisterIdForClass(DM, resourceClass);
      ...
      if (IsDynamicResourceShaderModel(DM)) {
        ...
        Constant *CreateHandleFromBindingOpcodeArg = HlslOP->GetU32Const(
            (unsigned)DXIL::OpCode::CreateHandleFromBinding);
        ...
        Value *createHandleFromBindingArgs[] = {
            CreateHandleFromBindingOpcodeArg, bindingV, registerIndex,
            isUniformRes};

        auto *handle = Builder.CreateCall(CreateHandleFromBindingOpFunc,
                                          createHandleFromBindingArgs, name);

        Function *annotHandleFn = HlslOP->GetOpFunc(
            DXIL::OpCode::AnnotateHandle, Type::getVoidTy(Ctx));
        ...
        Value *propertiesV = resource_helper::getAsConstant(
            RP, resPropertyTy, *DM.GetShaderModel());
        ...
        Constant *MetaDataArg =
            HlslOP->GetU32Const(resourceMetaDataId); // position of the metadata record in the
