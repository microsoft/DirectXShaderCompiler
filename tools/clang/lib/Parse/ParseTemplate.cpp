...
bool Parser::AnnotateTemplateIdToken(TemplateTy Template, TemplateNameKind TNK,
  ...
  assert(Template && (Tok.is(tok::less) || getLangOpts().HLSL) && // HLSL Change
  ...
  // HLSL Change Starts - allow template names without '<>'
  bool Invalid;
  bool ShouldParse = Tok.is(tok::less) || !getLangOpts().HLSL;
  if (ShouldParse) {  // HLSL Change - make following conditional
    Invalid = ParseTemplateIdAfterTemplateName(Template, 
                                                    TemplateNameLoc,
                                                    SS, false, LAngleLoc,
                                                    TemplateArgs,
                                                    RAngleLoc);
    ...
    Invalid = false;
    RAngleLoc = LAngleLoc = Tok.getLocation();
  }
  // HLSL Change Ends
...
ParsedTemplateArgument Parser::ParseTemplateTemplateArgument() {
  ...
  if (SS.isSet() && Tok.is(tok::kw_template)) {
    // HLSL Change Starts
    if (getLangOpts().HLSL) {
      Diag(Tok, diag::err_hlsl_unsupported_construct) << "template template argument";
      return Result;
    }
    // HLSL Change Ends
...
Parser::ParseTemplateArgumentList(TemplateArgList &TemplateArgs) {
  ...
  do {
    ...
    } // HLSL Change - end conditional block
