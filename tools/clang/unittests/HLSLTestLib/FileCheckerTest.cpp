...
#else
    FileRunCommandResult result = FileRunCommandResult::Success(
        "Can't run dxl on non-windows, so just assuming success");
    result.AbortPipeline = true;
    return result;
...
static IncludeHandlerVFSOverlayForTest *
...
FileRunCommandPart::RunDxcHashTest(dxc::DxcDllSupport &DllSupport) {
  ...
  if (pHash0->GetBufferSize() != pHash1->GetBufferSize() ||
      0 != memcmp(pHash0->GetBufferPointer(), pHash0->GetBufferPointer(),
