        ...
        for (const Instruction &I : BB) {
          ...
          case DXIL::OpCode::LinAlgMatrixAccumulateToMemory: {
            ...
            if (OpCode == DXIL::OpCode::LinAlgMatrixAccumulateToDescriptor) {
              ...
              MatrixIdx = static_cast<unsigned>(
                  DxilInst_LinAlgMatrixAccumulateToDescriptor::arg_matrix);
            } else {
              Flag = static_cast<uint8_t>(
                  PSVLinAlgAccumulateStoreFlag::GroupShared);
              MatrixIdx = static_cast<unsigned>(
                  DxilInst_LinAlgMatrixAccumulateToMemory::arg_matrix);
            }

            LinAlgMatrixInfo Matrix;
            if (!GetLinAlgMatrixInfo(CI->getArgOperand(MatrixIdx)->getType(),
                                     Matrix))
              break;
            DXIL::ComponentType AccumulatorType = Matrix.Type;
            // TODO: this needs to be deleted after the remove TargetType PR
            // lands
