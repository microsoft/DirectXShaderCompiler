...
#define CALL_MACRO_N_(PREFIX, N, ...) CONCAT(PREFIX, N)(__VA_ARGS__)
#define CALL_MACRO_N(PREFIX, ...) CALL_MACRO_N_(PREFIX, NARG(__VA_ARGS__), __VA_ARGS__)
...
#define VERIFY_SUCCEEDED(...) CALL_MACRO_N(VERIFY_SUCCEEDED_, __VA_ARGS__)

#define VERIFY_ARE_EQUAL_2(A, B) EXPECT_EQ(A, B)
#define VERIFY_ARE_EQUAL_3(A, B, msg) EXPECT_EQ(A, B) << msg
#define VERIFY_ARE_EQUAL(...) CALL_MACRO_N(VERIFY_ARE_EQUAL_, __VA_ARGS__)
...
#define TEST_CLASS_SETUP(method) bool method(); virtual void SetUp() { EXPECT_TRUE(method()); }
#define TEST_CLASS_CLEANUP(method) bool method(); virtual void TearDown() { EXPECT_TRUE(method()); }
...
// gtest lacks any module setup/cleanup. These functions are called by the main() function
// before and after tests are run. This approximates the behavior.
bool moduleSetup();
bool moduleTeardown();
#define MODULE_SETUP(method) bool method(); bool moduleSetup() { return method(); }
#define MODULE_CLEANUP(method) bool method(); bool moduleTeardown() { return method(); }

// No need to expand env vars on Unix platforms, so convert the slashes instead.
inline DWORD ExpandEnvironmentStringsW(_In_ LPCWSTR lpSrc,_Out_opt_ LPWSTR lpDst, _In_ DWORD nSize) {
  unsigned i;
  bool wasSlash = false;
...
namespace Common {
  ...
  public:
    ...
    int CompareNoCase(std::wstring str) const {return compare(str);}
...
namespace TestExecution {
  enum class VerifyOutputSettings {
    LogOnlyFailures
  };
  ...
  } // RuntimeParameters namespace
} // TestExecution namespace
namespace Logging {
namespace Log {
  inline void StartGroup(const wchar_t *name) {};
  inline void EndGroup(const wchar_t *name) {};
  inline void Comment(const wchar_t *msg) {fputws(msg, stdout); fputwc(L'\n', stdout);}
  inline void Error(const wchar_t *msg) {fputws(msg, stderr); fputwc(L'\n', stderr); ADD_FAILURE(); }
} // Log namespace
} // Logging namespace
} // WEX namespace
