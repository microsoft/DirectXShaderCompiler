...
// CHECK: VarDecl 0x[[t22:[0-9a-f]+]] {{.*}} used t22 'const test2':'const test<float, 1>'
test2 t22;
...
// CHECK: VarDecl 0x[[tt:[0-9a-f]+]] {{.*}} used tt 'const test3<test2>':'const test3<test<float, 1> >'
test3<test2>  tt;
...
// CHECK-NEXT: MemberExpr 0x{{.*}} 'const float':'const float' lvalue .d 0x[[field_d]]
// CHECK-NEXT: MemberExpr 0x{{.*}} <col:10, col:13> 'const test<float, 1>':'const test<float, 1>' lvalue .d 0x[[test3_d]]
// CHECK-NEXT: DeclRefExpr 0x{{.*}} <col:10> 'const test3<test2>':'const test3<test<float, 1> >' lvalue Var 0x[[tt]] 'tt' 'const test3<test2>':'const test3<test<float, 1> >'
// CHECK-NEXT: ImplicitCastExpr 0x{{.*}} 'float':'float' <LValueToRValue>
// CHECK-NEXT: MemberExpr 0x{{.*}} 'const float':'const float' lvalue .d 0x[[field_d]]
// CHECK-NEXT: DeclRefExpr 0x{{.*}} 'const test2':'const test<float, 1>' lvalue Var 0x[[t22]] 't22' 'const test2':'const test<float, 1>'
