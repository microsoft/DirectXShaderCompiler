...
HRESULT DxilContainerReader::Load(_In_ const void *pContainer,
                                  _In_ uint32_t containerSizeInBytes) {
...
HRESULT DxilContainerReader::GetPartContent(uint32_t idx,
                                            _Outptr_ const void **ppResult,
                                            _Out_ uint32_t *pResultSize) {
  if (ppResult == nullptr)
    return E_POINTER;
  *ppResult = nullptr;
  if (!IsLoaded())
...
HRESULT DxilContainerReader::GetPartFourCC(uint32_t idx,
                                           _Out_ uint32_t *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  if (!IsLoaded())
    return E_NOT_VALID_STATE;
