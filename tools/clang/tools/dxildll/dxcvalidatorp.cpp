...
public:
  ...
  HRESULT STDMETHODCALLTYPE ValidateWithDebug(
    _In_ IDxcBlob *pShader,                       // Shader to validate.
    _In_ UINT32 Flags,                            // Validation flags.
    _In_ DxcBuffer *pDebugModule,                 // Debug module bitcode to provide line numbers
    _COM_Outptr_ IDxcOperationResult **ppResult   // Validation output status, buffer, and errors
    ) override;
...
static void HashAndUpdate(DxilContainerHeader *pContainer) {
  ...
  static const UINT32 DXBCHashStartOffset = offsetof(struct DxilContainerHeader,Version); 
...
static void HashAndUpdateOrCopy(UINT32 Flags, IDxcBlob *pShader, IDxcBlob **pSigned) {
  if (Flags & DxcValidatorFlags_InPlaceEdit) {
    ...
    *pSigned = pShader;
    ...
    ULONG cb;
    IFT(pSignedBlobStream->Write(pShader->GetBufferPointer(), pShader->GetBufferSize(), &cb));
...
HRESULT STDMETHODCALLTYPE DxcValidator::ValidateWithDebug(
  _In_ IDxcBlob *pShader,                       // Shader to validate.
  _In_ UINT32 Flags,                            // Validation flags.
  _In_opt_ DxcBuffer *pOptDebugBitcode,         // Debug module bitcode to provide line numbers
  _COM_Outptr_ IDxcOperationResult **ppResult   // Validation output status, buffer, and errors
)
{
  DxcThreadMalloc TM(m_pMalloc);
  ...
  *ppResult = nullptr;
  ...
  try {
    ...
    if (Flags & DxcValidatorFlags_RootSignatureOnly) {
      validationStatus = RunRootSignatureValidation(pShader, pDiagStream, Flags, &pSignedBlob);
    }
    else {
      validationStatus = RunValidation(pShader, pDiagStream, Flags, pOptDebugBitcode, &pSignedBlob);
    }
    ...
    IFT(DxcResult::Create(validationStatus, DXC_OUT_OBJECT, {
...
HRESULT DxcValidator::RunValidation(_In_ IDxcBlob *pShader,
  ...
  *pSigned = nullptr;
  ...
  if (IsDxilContainerLike(pShader->GetBufferPointer(), pShader->GetBufferSize())) {
    IFR(ValidateDxilContainer(pShader->GetBufferPointer(), pShader->GetBufferSize(),
                              pDebugBitcode ? pDebugBitcode->Ptr : nullptr,
                              pDebugBitcode ? (uint32_t)pDebugBitcode->Size : 0,
                              DiagStream));
  }
  else {
...
HRESULT DxcValidator::RunRootSignatureValidation(
  ...
  try {
    ...
    if (pProgramHeader) {
      ...
      // Do not sign here; shaders must go through full shader validation for signing.
    } else {
