...
SpirvVariable *DeclResultIdMapper::createSpirvStageVar(
  ...
  // According to DXIL spec, the StartVertexLocation SV can only be used by VSIn.
  // According to Vulkan spec, the BaseVertex BuiltIn can only be used by VSIn.
  case hlsl::Semantic::Kind::StartVertexLocation: {
    stageVar->setIsSpirvBuiltin();
    return spvBuilder.addStageBuiltinVar(type, sc, BuiltIn::BaseVertex,
                                         isPrecise, srcLoc);
  }
  // According to DXIL spec, the StartInstanceLocation SV can only be used by VSIn.
  // According to Vulkan spec, the BaseInstance BuiltIn can only be used by VSIn.
