...
public:
  ...
  HRESULT STDMETHODCALLTYPE QueryInterface(REFIID iid,
    ...
    return ::SetupAndRun(
        m_pMalloc, std::mem_fn(&Entrypoint<IInterface>::QueryInterfaceImpl),
...
struct IDxcPixTypeEntrypoint : public Entrypoint<IDxcPixType> {
  ...
  STDMETHODIMP GetName(_Outptr_result_z_ BSTR *Name) override {
    return InvokeOnReal(&IInterface::GetName, CheckNotNull(OutParam(Name)));
  }

  STDMETHODIMP GetSizeInBits(_Outptr_result_z_ DWORD *SizeInBits) override {
    return InvokeOnReal(&IInterface::GetSizeInBits,
                        CheckNotNull(OutParam(SizeInBits)));
  }

  STDMETHODIMP UnAlias(_Outptr_result_z_ IDxcPixType **ppBaseType) override {
...
struct IDxcPixArrayTypeEntrypoint : public Entrypoint<IDxcPixArrayType> {
  ...
  STDMETHODIMP GetNumElements(_Outptr_result_z_ DWORD *ppNumElements) override {
    return InvokeOnReal(&IInterface::GetNumElements,
                        CheckNotNull(OutParam(ppNumElements)));
  }

  STDMETHODIMP
  GetIndexedType(_Outptr_result_z_ IDxcPixType **ppElementType) override {
    return InvokeOnReal(&IInterface::GetIndexedType,
                        CheckNotNull(OutParam(ppElementType)));
  }

  STDMETHODIMP
  GetElementType(_Outptr_result_z_ IDxcPixType **ppElementType) override {
...
struct IDxcPixStructFieldEntrypoint : public Entrypoint<IDxcPixStructField> {
  ...
  STDMETHODIMP GetType(_Outptr_result_z_ IDxcPixType **ppType) override {
    return InvokeOnReal(&IInterface::GetType, CheckNotNull(OutParam(ppType)));
  }

  STDMETHODIMP
  GetOffsetInBits(_Outptr_result_z_ DWORD *pOffsetInBits) override {
...
struct IDxcPixStructTypeEntrypoint : public Entrypoint<IDxcPixStructType> {
  ...
  STDMETHODIMP GetNumFields(_Outptr_result_z_ DWORD *ppNumFields) override {
    return InvokeOnReal(&IInterface::GetNumFields,
                        CheckNotNull(OutParam(ppNumFields)));
  }

  STDMETHODIMP
  GetFieldByIndex(DWORD dwIndex,
...
struct IDxcPixDxilStorageEntrypoint : public Entrypoint<IDxcPixDxilStorage> {
  ...
  STDMETHODIMP Index(_In_ DWORD Index,
                     _COM_Outptr_ IDxcPixDxilStorage **ppResult) override {
    return InvokeOnReal(&IInterface::Index, Index,
                        CheckNotNull(OutParam(ppResult)));
  }

  STDMETHODIMP GetRegisterNumber(_Outptr_result_z_ DWORD *pRegNum) override {
...
struct IDxcPixVariableEntrypoint : public Entrypoint<IDxcPixVariable> {
  ...
  STDMETHODIMP
  GetStorage(_COM_Outptr_ IDxcPixDxilStorage **ppStorage) override {
...
struct IDxcPixDxilLiveVariablesEntrypoint
  ...
  STDMETHODIMP GetCount(_Outptr_ DWORD *dwSize) override {
...
struct IDxcPixDxilDebugInfoEntrypoint
  ...
  STDMETHODIMP IsVariableInRegister(_In_ DWORD InstructionOffset,
  ...
  STDMETHODIMP GetStackDepth(_In_ DWORD InstructionOffset,
...
struct IDxcPixDxilSourceLocationsEntrypoint
  ...
  STDMETHODIMP GetFileNameByIndex(_In_ DWORD Index,
...
#define HANDLE_INTERFACE(IInterface)                                           \
  if (__uuidof(IInterface) == riid) {                                          \
    return NewDxcPixDxilDebugInfoObjectOrThrow<IInterface##Entrypoint>(        \
        (IInterface **)ppvObject, pMalloc, (IInterface *)pReal);               \
  }                                                                            \
  (void)0
