...
public:
    ...
    void Init( ID3D12Device* device, int raygenCount, int missCount, int hitGroupCount, int rayTypeCount, int rootTableDwords )
    {
        ...
        m_shaderRecordSizeInBytes = ROUND_UP( m_rootTableSizeInBytes + SHADER_ID_SIZE_IN_BYTES, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT );
        m_missStartIdx            = m_raygenCount;
        m_hitGroupStartIdx        = m_missStartIdx + m_missCount;

        const int m_totalSizeInBytes = (m_raygenCount+m_missCount+m_hitGroupCount) * m_shaderRecordSizeInBytes;

        D3D12_RESOURCE_DESC desc = CD3DX12_RESOURCE_DESC::Buffer(m_totalSizeInBytes, D3D12_RESOURCE_FLAG_NONE, std::max(D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT));
        CD3DX12_HEAP_PROPERTIES heap = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
        VERIFY_SUCCEEDED(device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, nullptr, IID_PPV_ARGS(&m_sbtResource)));
        m_sbtResource->SetName(L"SBT Resource Heap");
        CD3DX12_HEAP_PROPERTIES upload = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        VERIFY_SUCCEEDED(device->CreateCommittedResource(&upload, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_sbtUploadResource)));
    ...
    {
        CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(m_sbtResource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_DEST);
        cmdlist->ResourceBarrier(1, &barrier);
        cmdlist->CopyResource( m_sbtResource, m_sbtUploadResource );
        CD3DX12_RESOURCE_BARRIER barrier2 = CD3DX12_RESOURCE_BARRIER::Transition(m_sbtResource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    ...
    int   GetMissShaderRecordIdx( int idx, int rayType )      { return m_missStartIdx + idx*m_rayTypeCount + rayType; }
    int   GetHitGroupShaderRecordIdx( int idx, int rayType )  { return m_hitGroupStartIdx + idx*m_rayTypeCount + rayType; }

    void* GetRaygenShaderIdPtr( int idx )                     { return m_hostPtr + GetRaygenShaderRecordIdx(idx) * m_shaderRecordSizeInBytes; }
    void* GetMissShaderIdPtr( int idx, int rayType )          { return m_hostPtr + GetMissShaderRecordIdx(idx,rayType) * m_shaderRecordSizeInBytes; }
    void* GetHitGroupShaderIdPtr( int idx, int rayType )      { return m_hostPtr + GetHitGroupShaderRecordIdx(idx,rayType) * m_shaderRecordSizeInBytes; }
    ...
    int   GetRaygenRangeInBytes()                             { return m_raygenCount * m_shaderRecordSizeInBytes; }
    int   GetMissRangeInBytes()                               { return m_missCount * m_shaderRecordSizeInBytes; }
    int   GetHitGroupRangeInBytes()                           { return m_hitGroupCount * m_shaderRecordSizeInBytes; }

    D3D12_GPU_VIRTUAL_ADDRESS GetRaygenStartGpuVA()           { return m_sbtResource->GetGPUVirtualAddress() + GetRaygenShaderRecordIdx(0) * m_shaderRecordSizeInBytes; }
    D3D12_GPU_VIRTUAL_ADDRESS GetMissStartGpuVA()             { return m_sbtResource->GetGPUVirtualAddress() + GetMissShaderRecordIdx(0,0) * m_shaderRecordSizeInBytes; }
    D3D12_GPU_VIRTUAL_ADDRESS GetHitGroupStartGpuVA()         { return m_sbtResource->GetGPUVirtualAddress() + GetHitGroupShaderRecordIdx(0,0) * m_shaderRecordSizeInBytes; }
...
//-----------------------------------------------------------------------------
void AllocateBuffer( ID3D12Device *pDevice, UINT64 bufferSize, ID3D12Resource **ppResource, bool allowUAV=false, D3D12_RESOURCE_STATES initialResourceState = D3D12_RESOURCE_STATE_COMMON, const wchar_t* resourceName = nullptr)
{
    auto uploadHeapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    auto bufferDesc = CD3DX12_RESOURCE_DESC::Buffer(bufferSize, allowUAV ? D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS : D3D12_RESOURCE_FLAG_NONE );
...
void ReallocScratchResource(ID3D12Device *pDevice, ID3D12Resource **ppResource, UINT64 nbytes)
{
    ...
    {
        AllocateBuffer(pDevice, nbytes, ppResource, true, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, L"scratchResource");
    }
}

//-----------------------------------------------------------------------------
void AllocateUploadBuffer(ID3D12Device *pDevice, const void *pData, UINT64 datasize, ID3D12Resource **ppResource, const wchar_t* resourceName = nullptr)
{
...
//-----------------------------------------------------------------------------
void AllocateBufferFromUpload(ID3D12Device *pDevice, ID3D12GraphicsCommandList *pCommandList, ID3D12Resource* uploadSource, ID3D12Resource **ppResource, D3D12_RESOURCE_STATES targetResourceState, const wchar_t* resourceName = nullptr)
{
    const bool allowUAV = targetResourceState == D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
    AllocateBuffer(pDevice, uploadSource->GetDesc().Width, ppResource, allowUAV, D3D12_RESOURCE_STATE_COPY_DEST, resourceName);
    pCommandList->CopyResource(*ppResource, uploadSource);
    CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(*ppResource, D3D12_RESOURCE_STATE_COPY_DEST, targetResourceState);
...
// MARKER: ExecutionTest/DxilConf Shared Implementation Start
 ...
 // This is defined in d3d.h for Windows 10 Anniversary Edition SDK, but we only
  ...
  void RunPAQTest(ID3D12Device *pDevice, LPCSTR shader, D3D_SHADER_MODEL shaderModel, LPCWSTR *pOptions, int numOptions, std::vector<int> &testData);
...
void ExecutionTest::RunPAQTest(ID3D12Device* pDevice0, LPCSTR shader, D3D_SHADER_MODEL shaderModel, LPCWSTR* pOptions, int numOptions, std::vector<int> &testData)
{
    // Running the PAQ test will require SM6.6+, so _HLK_CONF may need to be defined in order to run with a graphics adpater driver.
    CComPtr<ID3D12Device5> pDevice;
    VERIFY_SUCCEEDED(pDevice0->QueryInterface(IID_PPV_ARGS(&pDevice)));
    ...
    LPCWSTR pTargetProfile;
    switch (shaderModel) {
        default: pTargetProfile = L"lib_6_3"; break; // Default to SM 6.3, which must skip PAQ testing.
        case D3D_SHADER_MODEL_6_6: pTargetProfile = L"lib_6_6"; break;
        case D3D_SHADER_MODEL_6_7: pTargetProfile = L"lib_6_7"; break;
    }
    ...
    CComPtr<ID3D12DescriptorHeap> pDescriptorHeap;
    {
        ...
        pDevice->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(&pDescriptorHeap));
        pDescriptorHeap->SetName(L"Descriptor Heap");
    }
    int descriptorSize = pDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    // Testbuffer
    {
        auto resDesc = CD3DX12_RESOURCE_DESC::Buffer(testData.size() * sizeof( int ), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
        auto defaultHeapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
        VERIFY_SUCCEEDED(pDevice->CreateCommittedResource(&defaultHeapProperties, D3D12_HEAP_FLAG_NONE, &resDesc, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&pTestBuffer)));
        pTestBuffer->SetName(L"Test Buffer");

        const int descriptorIndex = 0;
        D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptorHandle = CD3DX12_CPU_DESCRIPTOR_HANDLE(pDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), descriptorIndex, descriptorSize);
        D3D12_UNORDERED_ACCESS_VIEW_DESC UAVDesc = {};
        ...
        UAVDesc.Buffer.CounterOffsetInBytes = 0;
        UAVDesc.Buffer.Flags = D3D12_BUFFER_UAV_FLAG_NONE;
        pDevice->CreateUnorderedAccessView(pTestBuffer, nullptr, &UAVDesc, cpuDescriptorHandle);
    }
    ...
    // Testbuffer Readback
    {
        ...
        pDevice->CreateCommittedResource(&readHeap, D3D12_HEAP_FLAG_NONE, &readDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&pTestBufferRead));
    }

    // Create CBV resource (sceneConstantBuffer), index 1
    {
        const int descriptorIndex = 1;
        const UINT constantBufferSize = (sizeof(SceneConsts) + (D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1))
        ...
        pSceneConstantBuffer->Map(0, &readRange, reinterpret_cast<void**>(&sceneConstantBufferWO));
    ...
    CComPtr<ID3D12RootSignature> pLocalRootSignature;
    {
        ...
        CD3DX12_ROOT_PARAMETER   rootParameters[1];
        bufferRanges[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 1, 0, 2);  // vertexBuffer(t1), indexBuffer(t2)
        rootParameters[0].InitAsDescriptorTable(_countof( bufferRanges ), bufferRanges, D3D12_SHADER_VISIBILITY_ALL);

        CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc;
        rootSignatureDesc.Init(_countof(rootParameters), rootParameters, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_LOCAL_ROOT_SIGNATURE);
        CComPtr<ID3DBlob> signature;
        CComPtr<ID3DBlob> error;
        VERIFY_SUCCEEDED( D3D12SerializeRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature, &error));
        VERIFY_SUCCEEDED( pDevice->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&pLocalRootSignature)));
    ...
    CComPtr<ID3D12RootSignature> pGlobalRootSignature;
    {
        ...
        bufferRanges[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0);                     // testBuffer(u0)
        rootParameters[0].InitAsShaderResourceView(0, 0, D3D12_SHADER_VISIBILITY_ALL);   // accelStruct(t0)
        rootParameters[1].InitAsConstantBufferView(0);                                   // sceneConstants(b0)
        rootParameters[2].InitAsDescriptorTable(_countof(bufferRanges), bufferRanges, D3D12_SHADER_VISIBILITY_ALL);
        ...
        CComPtr<ID3DBlob> error;
        VERIFY_SUCCEEDED(D3D12SerializeRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature, &error));
        VERIFY_SUCCEEDED(pDevice->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&pGlobalRootSignature)));
        pGlobalRootSignature->SetName(L"Global Root Signature");
    }

    // Create command queue.
    CComPtr<ID3D12CommandQueue> pCommandQueue;
    CreateCommandQueue(pDevice, L"RunPAQTest Command Queue", &pCommandQueue, D3D12_COMMAND_LIST_TYPE_DIRECT);

    // Compile raygen shader.
    CComPtr<ID3DBlob> pShaderLib;
    CompileFromText(shader, L"rg_read_write", pTargetProfile, &pShaderLib, pOptions, numOptions);
    ...
    auto globalRootSigSubObj = stateObjectDesc.CreateSubobject<CD3DX12_GLOBAL_ROOT_SIGNATURE_SUBOBJECT>();
    ...
    stateObjectDesc.CreateSubobject<CD3DX12_LOCAL_ROOT_SIGNATURE_SUBOBJECT>()->SetRootSignature(pLocalRootSignature);
    stateObjectDesc.CreateSubobject<CD3DX12_GLOBAL_ROOT_SIGNATURE_SUBOBJECT>()->SetRootSignature(pGlobalRootSignature);
    ...
    memcpy(shaderTable.GetRaygenShaderIdPtr(0),     pStateObjectProperties->GetShaderIdentifier(L"rg_read_write"), SHADER_ID_SIZE_IN_BYTES);
    memcpy(shaderTable.GetMissShaderIdPtr(0,0),     pStateObjectProperties->GetShaderIdentifier(L"miss_read_write"), SHADER_ID_SIZE_IN_BYTES);
    memcpy(shaderTable.GetHitGroupShaderIdPtr(0,0), pStateObjectProperties->GetShaderIdentifier(L"HitGroup"), SHADER_ID_SIZE_IN_BYTES);
    ...
    AllocateUploadBuffer(pDevice, verts, vertexDataSize, &vertexBufferUpload, L"vertexBufferUpload");
    AllocateUploadBuffer(pDevice, indices, indexDataSize,  &indexBufferUpload,  L"indexBufferUpload");

    AllocateBufferFromUpload(pDevice, pCommandList, vertexBufferUpload, &vertexBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, L"vertexBuffer");
    AllocateBufferFromUpload(pDevice, pCommandList, indexBufferUpload, &indexBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, L"indexBuffer");

    {
        ...
        srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        srvDesc.Buffer.NumElements = UINT(vertexDataSize / sizeof(DirectX::XMFLOAT3));
        srvDesc.Format = DXGI_FORMAT_UNKNOWN;
        srvDesc.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_NONE;
        srvDesc.Buffer.StructureByteStride = sizeof(DirectX::XMFLOAT3);
        pDevice->CreateShaderResourceView(vertexBuffer, &srvDesc, cpuDescriptorHandle);
    }
    {
        ...
        pDevice->CreateShaderResourceView(indexBuffer, &srvDesc, cpuDescriptorHandle);
    }    
    ...
    // Build BLAS.
    {
        ...
        geometryDesc.Triangles.IndexCount = static_cast<UINT>(indexBuffer->GetDesc().Width) / sizeof(int);
        geometryDesc.Triangles.IndexFormat = DXGI_FORMAT_R32_UINT;
        geometryDesc.Triangles.Transform3x4 = 0;
        geometryDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
        geometryDesc.Triangles.VertexCount = static_cast<UINT>(vertexBuffer->GetDesc().Width) / sizeof(DirectX::XMFLOAT3);
        ...
        geometryDesc.Flags = D3D12_RAYTRACING_GEOMETRY_FLAG_NONE; // Non-opaque to trigger anyhit.

        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE;

        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS accelInputs = {};
        accelInputs.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
        ...
        pDevice->GetRaytracingAccelerationStructurePrebuildInfo(&accelInputs, &prebuildInfo);

        ReallocScratchResource(pDevice, &scratchResource, prebuildInfo.ScratchDataSizeInBytes);
        AllocateBuffer(pDevice, prebuildInfo.ResultDataMaxSizeInBytes, &blasResource, true, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, L"BLAS");

        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC buildDesc = {};
        buildDesc.Inputs = accelInputs;
        buildDesc.ScratchAccelerationStructureData = scratchResource->GetGPUVirtualAddress();
        buildDesc.DestAccelerationStructureData = blasResource->GetGPUVirtualAddress();

        pCommandList->BuildRaytracingAccelerationStructure(&buildDesc,0,0);
        CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::UAV(blasResource);
        pCommandList->ResourceBarrier(1, (const D3D12_RESOURCE_BARRIER*)&barrier);
    }
    ...
    // Build TLAS.
    {
        ...
        AllocateUploadBuffer(pDevice, &instanceDesc, sizeof(instanceDesc), &instanceDescs, L"instanceDescs");
        ...
        AllocateBuffer(pDevice, prebuildInfo.ResultDataMaxSizeInBytes, &tlasResource, true, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, L"TLAS");

        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC buildDesc = {};
        buildDesc.Inputs = accelInputs;
        buildDesc.ScratchAccelerationStructureData = scratchResource->GetGPUVirtualAddress();
        buildDesc.DestAccelerationStructureData = tlasResource->GetGPUVirtualAddress();

        pCommandList->BuildRaytracingAccelerationStructure(&buildDesc,0,0);
        CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::UAV(tlasResource);
        pCommandList->ResourceBarrier(1, (const D3D12_RESOURCE_BARRIER*)&barrier);
    }
    ...
    pCommandList->SetComputeRootShaderResourceView(0, tlasResource->GetGPUVirtualAddress());
    pCommandList->SetComputeRootConstantBufferView(1, pSceneConstantBuffer->GetGPUVirtualAddress());
    pCommandList->SetComputeRootDescriptorTable(2, pDescriptorHeap->GetGPUDescriptorHandleForHeapStart());


    D3D12_DISPATCH_RAYS_DESC dispatchDesc = {};
    dispatchDesc.RayGenerationShaderRecord.StartAddress = shaderTable.GetRaygenStartGpuVA();
    dispatchDesc.RayGenerationShaderRecord.SizeInBytes  = shaderTable.GetRaygenRangeInBytes();
    ...
    dispatchDesc.MissShaderTable.StrideInBytes          = shaderTable.GetShaderRecordSizeInBytes();
    dispatchDesc.HitGroupTable.StartAddress             = shaderTable.GetHitGroupStartGpuVA();
    dispatchDesc.HitGroupTable.SizeInBytes              = shaderTable.GetHitGroupRangeInBytes();
    dispatchDesc.HitGroupTable.StrideInBytes            = shaderTable.GetShaderRecordSizeInBytes();
    ...
    barriers[0] = CD3DX12_RESOURCE_BARRIER::Transition(pTestBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
    pCommandList->ResourceBarrier(1, barriers);
    pCommandList->CopyResource(pTestBufferRead, pTestBuffer);
    barriers[0] = CD3DX12_RESOURCE_BARRIER::Transition(pTestBuffer, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    pCommandList->ResourceBarrier(1, barriers);
...
void ch_read(inout PerRayData payload, in Attrs attrs)
{
    ...
    if (!bSM_6_6_Supported) {
        WEX::Logging::Log::Comment(L"PAQTest requires shader model 6.6+ but no supported device was found.");
        WEX::Logging::Log::Result(WEX::Logging::TestResults::Skipped);
        return;
    }
    bool bDXRSupported = bSM_6_6_Supported && DoesDeviceSupportRayTracing(pDevice);
    ...
    if (bDXRSupported) {
        if (bSM_6_6_Supported) {
            ...
            RunPAQTest(pDevice, pShader, D3D_SHADER_MODEL_6_6, optsSM66, _countof(optsSM66), testData);
