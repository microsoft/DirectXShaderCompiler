...
Type *LowerWaveMatType::lowerType(Type *Ty) {
  if (Ty->isPointerTy()) {
    return PointerType::get(lowerType(Ty->getPointerElementType()), Ty->getPointerAddressSpace());
  } else if (Ty->isArrayTy()) {
...
static CallInst *RewriteIntrinsicCallForNewArg(CallInst *CI, Value *OldV, Value *NewV, Type *NewRet = nullptr) {
...
INITIALIZE_PASS(LowerWaveMatType, "hlsl-lower-wavematrix-type",
  "Lower WaveMatrix types to dxil type", false,
  false)
