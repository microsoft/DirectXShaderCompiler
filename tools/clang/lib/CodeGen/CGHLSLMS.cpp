  ...
  for (; ArgNo < F->arg_size(); ++ArgNo, ++ParmIdx, ++ArgIt) {
    ...
    if (isRay) {
      ...
      case DXIL::ShaderKind::Intersection:
        // RayGeneration and Intersection shaders are not allowed to have any
        // input parameters
        Diags.Report(
            parmDecl->getLocation(),
            Diags.getCustomDiagID(DiagnosticsEngine::Error,
                                  "parameters are not allowed for %0 shader"))
            << (funcProps->shaderKind == DXIL::ShaderKind::RayGeneration
                    ? "raygeneration"
                    : "intersection");
      ...
      case DXIL::ShaderKind::ClosestHit:
        if (0 == ArgNo && dxilInputQ != DxilParamInputQual::Inout) {
          ...
          Diags.Report(parmDecl->getLocation(),
                       Diags.getCustomDiagID(
                           DiagnosticsEngine::Error,
          ...
          Diags.Report(
              parmDecl->getLocation(),
              Diags.getCustomDiagID(DiagnosticsEngine::Error,
                                    "too many parameters, expected payload and "
                                    "attributes parameters only."));
          rayShaderHaveErrors = true;
        }
        if (ArgNo < 2) {
          if (!IsHLSLCopyableAnnotatableRecord(parmDecl->getType())) {
            Diags.Report(parmDecl->getLocation(),
                         Diags.getCustomDiagID(
                             DiagnosticsEngine::Error,
                             "payload and attribute structures must be user "
                             "defined types with only numeric contents."));
            rayShaderHaveErrors = true;
          } else {
            DataLayout DL(&this->TheModule);
            unsigned size =
        ...
        if (ArgNo < 1) {
          if (!IsHLSLCopyableAnnotatableRecord(parmDecl->getType())) {
            Diags.Report(parmDecl->getLocation(),
                         Diags.getCustomDiagID(
                             DiagnosticsEngine::Error,
                             "ray payload parameter must be a user defined "
                             "type with only numeric contents."));
            rayShaderHaveErrors = true;
          } else {
            DataLayout DL(&this->TheModule);
            unsigned size =
        ...
        if (ArgNo < 1) {
          if (!IsHLSLCopyableAnnotatableRecord(parmDecl->getType())) {
            Diags.Report(parmDecl->getLocation(),
                         Diags.getCustomDiagID(
                             DiagnosticsEngine::Error,
                             "callable parameter must be a user defined type "
                             "with only numeric contents."));
            rayShaderHaveErrors = true;
          } else {
            DataLayout DL(&this->TheModule);
            unsigned size =
...
void CGMSHLSLRuntime::EmitHLSLAggregateStore(CodeGenFunction &CGF,
                                             llvm::Value *SrcVal,
                                             llvm::Value *DestPtr,
                                             clang::QualType Ty) {
  DXASSERT(
      0,
      "aggregate return type will use SRet, no aggregate store should exist");
}
