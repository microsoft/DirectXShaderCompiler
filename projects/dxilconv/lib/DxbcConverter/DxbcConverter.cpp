...
__override HRESULT STDMETHODCALLTYPE DxbcConverter::Convert(
    ...
    _Out_ UINT32 *pDxilSize, _Outptr_result_maybenull_z_ LPWSTR *ppDiag) {
...
__override HRESULT STDMETHODCALLTYPE DxbcConverter::ConvertInDriver(
    ...
    _In_opt_z_ LPCVOID pOutputSignature, _In_ UINT32 NumOutputSignatureElements,
...
void DxbcConverter::ConvertImpl(_In_reads_bytes_(DxbcSize) LPCVOID pDxbc,
                                ...
                                _In_opt_z_ LPCWSTR pExtraOptions,
                                _Outptr_result_bytebuffer_maybenull_(*pDxilSize)
                                    LPVOID *ppDxil,
...
void DxbcConverter::ConvertInDriverImpl(
    ...
    _In_ UINT32 NumPatchConstantSignatureElements,
    _In_opt_z_ LPCWSTR pExtraOptions, _Out_ IDxcBlob **ppDxcBlob,
    _Outptr_result_maybenull_z_ LPWSTR *ppDiag) {
  IFTARG(pByteCode);
  IFTARG(ppDxcBlob);
    ...
    case D3D10_SB_OPCODE_CUSTOMDATA:
      if (Inst.m_CustomData.Type ==
        ...
        llvm::Constant *pIcbData = ConstantDataArray::get(
            m_Ctx,
            ArrayRef<uint32_t>((uint32_t *)Inst.m_CustomData.pData, Size));
...
void DxbcConverter::LogConvertResult(
    bool InDriver, _In_ const LARGE_INTEGER *pQPCConvertStart,
    _In_ const LARGE_INTEGER *pQPCConvertEnd,
    _In_reads_bytes_(DxbcSize) LPCVOID pDxbc, _In_ UINT32 DxbcSize,
    _In_opt_z_ LPCWSTR pExtraOptions,
    _In_reads_bytes_(ConvertedSize) LPCVOID pConverted,
    _In_opt_ UINT32 ConvertedSize, HRESULT hr) {
