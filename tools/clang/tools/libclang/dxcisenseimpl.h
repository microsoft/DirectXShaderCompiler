...
public:
    ...
    HRESULT STDMETHODCALLTYPE
    GetLocation(IDxcFile *file, unsigned line, unsigned column,
                IDxcSourceLocation **pResult) override;
    ...
    HRESULT STDMETHODCALLTYPE GetCursorForLocation(
        IDxcSourceLocation *location, IDxcCursor **pResult) override;
    ...
    HRESULT STDMETHODCALLTYPE GetDiagnosticDetails(
        ...
        unsigned *errorCode, unsigned *errorLine, unsigned *errorColumn,
        BSTR *errorFile, unsigned *errorOffset, unsigned *errorLength,
        BSTR *errorMessage) override;
    HRESULT STDMETHODCALLTYPE
    GetInclusionList(unsigned *pResultCount, IDxcInclusion ***pResult) override;
    HRESULT STDMETHODCALLTYPE
    CodeCompleteAt(const char *fileName, unsigned line, unsigned column,
                   IDxcUnsavedFile **pUnsavedFiles, unsigned numUnsavedFiles,
                   DxcCodeCompleteFlags options,
                   IDxcCodeCompleteResults **pResult) override;
};
