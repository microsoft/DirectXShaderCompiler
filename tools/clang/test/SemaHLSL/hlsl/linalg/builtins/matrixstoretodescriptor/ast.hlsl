...
// CHECK: `-FunctionDecl {{[^ ]+}} <<invalid sloc>> <invalid sloc> implicit used __builtin_LinAlg_MatrixStoreToDescriptor 'void (__builtin_LinAlg_MatrixRef, RWByteAddressBuffer, unsigned int, unsigned int, unsigned int)' extern
...
// CHECK-NEXT: |-HLSLIntrinsicAttr {{[^ ]+}} <<invalid sloc>> Implicit "op" "" 414
// CHECK-NEXT: |-AvailabilityAttr {{[^ ]+}} <<invalid sloc>> Implicit  6.10 0 0 ""
// CHECK-NEXT: `-HLSLBuiltinCallAttr {{[^ ]+}} <<invalid sloc>> Implicit
