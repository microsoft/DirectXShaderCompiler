...
_Use_decl_annotations_ HRESULT DxilContainerReflection::GetPartContent(
  ...
  *ppResult = nullptr;
  ...
  uint32_t offset = (uint32_t)(
      pData -
      (char *)m_container->GetBufferPointer()); // Offset from the beginning.
  ...
  if (typeAnnotation.HasMatrixAnnotation()) {
    ...
    if (TryToDetectObjectType(structType, &m_Desc.Type)) {
      ...
      for (unsigned int ff = 0; ff < fieldCount; ++ff) {
        ...
        // Effectively, we want to add one to `Columns` for every scalar nested
        // recursively inside this `struct` type (ignoring objects, which we
        // filtered above). We should be able to compute this as the product of
        // the `Columns`, `Rows` and `Elements` of each field, with the caveat
        // that some of these may be zero, but shoud be treated as one.
        columnCounter +=
