...
static Constant *InsertAddrSpaceCastIfRequired(Constant *C, unsigned AddrSpace) {
  Type *Ty = C->getType();
  DXASSERT_NOMSG(Ty->isPointerTy());
  if (AddrSpace != Ty->getPointerAddressSpace()) {
    return ConstantExpr::getAddrSpaceCast(C,
              PointerType::get(Ty->getPointerElementType(), AddrSpace));
  }
  return C;
}
static Value *InsertAddrSpaceCastIfRequired(IRBuilder<> &Builder, Value *V, unsigned AddrSpace) {
  Type *Ty = V->getType();
  DXASSERT_NOMSG(Ty->isPointerTy());
  if (AddrSpace != Ty->getPointerAddressSpace()) {
    return Builder.CreateAddrSpaceCast(V, PointerType::get(Ty->getPointerElementType(), AddrSpace));
  }
  return V;
}
