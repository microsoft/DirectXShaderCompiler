...
// Correct plural/singular form in error message.
float4 f4_from_f2 = float(0, 0); // expected-error {{too many elements in vector initialization (expected 1 element, have 2)}} fxc-error {{X3014: incorrect number of arguments to numeric-type constructor}}
...
float4 f4c_f2_f = f3c_f2_f; // expected-error {{cannot initialize a variable of type 'vector<[...], 4>' with an lvalue of type 'vector<[...], 3>'}} fxc-error {{X3017: cannot implicitly convert from 'float3' to 'float4'}}
