...
void closesthit(
    ...
    // expected-note@24{{'dx::HitObject' field declared here}}
    // expected-error@+1{{HitObjects in entry function parameters are not supported}}
...
[shader("callable")]
void callable1(
    // expected-error@+2{{callable parameter 'p' must be a user-defined type composed of only numeric types}}
    // expected-error@+1{{HitObjects in entry function parameters are not supported}}
    inout RTTYPE p) { 
  CallShader(0, p); // expected-error{{HitObjects in user-defined struct parameter are not supported}}
}
...
void broadcast(
    // expected-error@+2{{object 'dx::HitObject' may not appear in a node record}}
    // expected-note@206{{'dx::HitObject' field declared here}}
