    ...
    while begin != -1:
        ...
        if begin != -1:
            ...
            if m:
                ...
                if tag:
                    ...
                    if m.group(3):
                        ...
                        try:
                            ...
                            print('%s(%d): Exception during eval("%s")' % (doc.name, text.count(native_endl, 0, begin) + 1, tag_code))
                            raise
                        tag_code = tag_code.replace('(_operation_function_)', tag)
                else:
                    tag = ''
                if m.group(5):      # we have form: <example_py[...]/>          # whole tag selected currently
                ...
                if end == -1:
                    print('%s(%d): Error: Could not find matching </py>' % (doc.name, text.count(native_endl, 0, begin) + 1))
                    begin = -1
                break
            else:
    ...
    if begin != -1 and end != -1:
        ...
        if op:
            ...
            try:
                ...
                print('%s(%d): Exception during eval("%s")' % (doc.name, doc.text.count(native_endl, 0, begin) + 1, code))
                raise
            ...
            try:
                ...
                print('%s(%d): Exception executing code' % (doc.name, doc.text.count(native_endl, 0, begin) + 1))
                raise
    return end
...
class Test(object):
    ...
    def SetDependencies():
        ...
        if not hasattr(globals(), 'app'):
            NormalizeForVSText, NormalizeForPython, TerminateTextForVS, WrapFunctionForExceptionHandling, ReplaceSelection = (
                Test.NormalizeForVSText, Test.NormalizeForPython, Test.TerminateTextForVS, Test.WrapFunctionForExceptionHandling, Test.ReplaceSelection)
            app = Test.TestApp()
...
def usage():
    print("""CodeTags.py - Templatize and generate code with python tags.
