...
// CHECK: [[sampled_arr1:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[load_arr1]] [[v3fc]] %float_2
    float val5 = tex2dArray.SampleCmp(float3(0.5, 0.25, 0), 2.0f);

// CHECK: [[tex1d_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_1d_sampled_image]] %tex1d
// CHECK: [[sampled_1d:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[tex1d_load]] %float_0_5 %float_2
    float val6 = tex1d.SampleCmp(0.5, 2.0f);

// CHECK: [[tex1da_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_1d_sampled_image_array]] %tex1dArray
// CHECK: [[sampled_1da:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[tex1da_load]] {{%[0-9]+}} %float_2
    float val7 = tex1dArray.SampleCmp(float2(0.5, 0), 2.0f);

// CHECK: [[cube_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_sampled_image]] %texCube
// CHECK: [[sampled_cube:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[cube_load]] {{%[0-9]+}} %float_2
    float val8 = texCube.SampleCmp(float3(0.5, 0.25, 0), 2.0f);

// CHECK: [[cube_arr_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_sampled_image_array]] %texCubeArray
// CHECK: [[sampled_cube_arr:%[a-zA-Z0-9_]+]] = OpImageSampleDrefImplicitLod %float [[cube_arr_load]] [[v4fc]] %float_2
    float val9 = texCubeArray.SampleCmp(float4(0.5, 0.25, 0, 0), 2.0f);
        return 1.0;
}
