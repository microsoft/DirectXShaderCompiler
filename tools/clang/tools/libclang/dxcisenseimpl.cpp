...
public:
  ...
  ~DxcBasicUnsavedFile();
  HRESULT Initialize(_In_z_ LPCSTR fileName, _In_z_ LPCSTR contents,
                     unsigned length);
  static HRESULT Create(_In_z_ LPCSTR fileName, _In_z_ LPCSTR contents,
                        unsigned length,
                        _COM_Outptr_ IDxcUnsavedFile **pObject);

  HRESULT STDMETHODCALLTYPE
  GetFileName(_Outptr_result_z_ LPSTR *pFileName) override;
  HRESULT STDMETHODCALLTYPE
  GetContents(_Outptr_result_z_ LPSTR *pContents) override;
...
static HRESULT AnsiToBSTR(_In_opt_z_ const char *text,
                          _Outptr_result_maybenull_ BSTR *pValue) throw() {
  if (pValue == nullptr)
    return E_POINTER;
  *pValue = nullptr;
  if (text == nullptr) {
...
static _Ret_opt_ _Post_readable_byte_size_(cb) __drv_allocatesMem(Mem) LPVOID
    CoTaskMemAllocZero(SIZE_T cb) throw() {
...
template <typename T>
static void
CoTaskMemAllocZeroElems(_In_ SIZE_T elementCount,
                        _Outptr_result_buffer_maybenull_(elementCount)
                            T **buf) throw() {
  *buf = reinterpret_cast<T *>(CoTaskMemAllocZero(elementCount * sizeof(T)));
}

static HRESULT
CXStringToAnsiAndDispose(CXString value,
                         _Outptr_result_maybenull_ LPSTR *pValue) throw() {
  if (pValue == nullptr)
    return E_POINTER;
  *pValue = nullptr;
...
static HRESULT
CXStringToBSTRAndDispose(CXString value,
                         _Outptr_result_maybenull_ BSTR *pValue) throw() {
...
static void CleanupUnsavedFiles(_In_count_(file_count) CXUnsavedFile *files,
...
static HRESULT CoTaskMemAllocString(_In_z_ const char *src,
                                    _Outptr_ LPSTR *pResult) throw() {
  assert(src != nullptr);
...
static void SafeReleaseIfaceArray(_Inout_count_(count) TIface **arr,
...
static HRESULT
SetupUnsavedFiles(_In_count_(num_unsaved_files) IDxcUnsavedFile **unsaved_files,
                  unsigned num_unsaved_files,
                  _Outptr_result_buffer_maybenull_(num_unsaved_files)
                      CXUnsavedFile **files) {
  *files = nullptr;
...
static HRESULT PagedCursorVisitorCopyResults(
    _In_ PagedCursorVisitorContext *context, _Out_ unsigned *pResultLength,
...
_Use_decl_annotations_ HRESULT DxcBasicUnsavedFile::Initialize(
...
DxcBasicUnsavedFile::Create(LPCSTR fileName, LPCSTR contents,
                            unsigned contentLength, IDxcUnsavedFile **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
_Use_decl_annotations_ HRESULT
DxcBasicUnsavedFile::GetFileName(LPSTR *pFileName) {
  return CoTaskMemAllocString(m_fileName, pFileName);
}

_Use_decl_annotations_ HRESULT
DxcBasicUnsavedFile::GetContents(LPSTR *pContents) {
...
_Use_decl_annotations_ HRESULT DxcCursor::Create(const CXCursor &cursor,
                                                 IDxcCursor **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
_Use_decl_annotations_ HRESULT DxcCursor::GetExtent(IDxcSourceRange **pValue) {
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetLocation(IDxcSourceLocation **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcSourceLocation::Create(clang_getCursorLocation(m_cursor), pResult);
}

_Use_decl_annotations_ HRESULT DxcCursor::GetKind(DxcCursorKind *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetKindFlags(DxcCursorKindFlags *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetSemanticParent(IDxcCursor **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcCursor::Create(clang_getCursorSemanticParent(m_cursor), pResult);
}

_Use_decl_annotations_ HRESULT
DxcCursor::GetLexicalParent(IDxcCursor **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcCursor::Create(clang_getCursorLexicalParent(m_cursor), pResult);
}

_Use_decl_annotations_ HRESULT DxcCursor::GetCursorType(IDxcType **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcType::Create(clang_getCursorType(m_cursor), pResult);
}

_Use_decl_annotations_ HRESULT DxcCursor::GetNumArguments(int *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcCursor::GetArgumentAt(int index,
                                                        IDxcCursor **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetReferencedCursor(IDxcCursor **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcCursor::Create(clang_getCursorReferenced(m_cursor), pResult);
}

_Use_decl_annotations_ HRESULT
DxcCursor::GetDefinitionCursor(IDxcCursor **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcCursor::Create(clang_getCursorDefinition(m_cursor), pResult);
}

_Use_decl_annotations_ HRESULT DxcCursor::FindReferencesInFile(
    IDxcFile *file, unsigned skip, unsigned top, unsigned *pResultLength,
    IDxcCursor ***pResult) {
...
_Use_decl_annotations_ HRESULT DxcCursor::GetSpelling(LPSTR *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT DxcCursor::GetDisplayName(BSTR *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetQualifiedName(BOOL includeTemplateArgs, BSTR *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetFormattedName(DxcCursorFormatting formatting, BSTR *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
  return CXStringToBSTRAndDispose(
...
_Use_decl_annotations_ HRESULT DxcCursor::GetChildren(unsigned skip,
                                                      unsigned top,
...
_Use_decl_annotations_ HRESULT
DxcCursor::GetSnappedChild(IDxcSourceLocation *location, IDxcCursor **pResult) {
  if (location == nullptr)
    return E_POINTER;
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcDiagnostic::Create(
    const CXDiagnostic &diagnostic, IDxcDiagnostic **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
_Use_decl_annotations_ HRESULT DxcDiagnostic::FormatDiagnostic(
    DxcDiagnosticDisplayOptions options, LPSTR *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT
DxcDiagnostic::GetLocation(IDxcSourceLocation **pResult) {
  return DxcSourceLocation::Create(clang_getDiagnosticLocation(m_diagnostic),
                                   pResult);
}

_Use_decl_annotations_ HRESULT DxcDiagnostic::GetSpelling(LPSTR *pResult) {
  return CXStringToAnsiAndDispose(clang_getDiagnosticSpelling(m_diagnostic),
                                  pResult);
}

_Use_decl_annotations_ HRESULT DxcDiagnostic::GetCategoryText(LPSTR *pResult) {
...
_Use_decl_annotations_ HRESULT
DxcDiagnostic::GetRangeAt(unsigned index, IDxcSourceRange **pResult) {
...
_Use_decl_annotations_ HRESULT DxcDiagnostic::GetFixItAt(
    unsigned index, IDxcSourceRange **pReplacementRange, LPSTR *pText) {
...
_Use_decl_annotations_ HRESULT DxcFile::Create(const CXFile &file,
                                               IDxcFile **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
_Use_decl_annotations_ HRESULT DxcFile::GetName(LPSTR *pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return CXStringToAnsiAndDispose(clang_getFileName(m_file), pResult);
}

_Use_decl_annotations_ HRESULT DxcFile::IsEqualTo(IDxcFile *other,
                                                  BOOL *pResult) {
  if (!pResult)
    return E_POINTER;
  if (other == nullptr) {
...
_Use_decl_annotations_ HRESULT DxcInclusion::Initialize(
    CXFile file, unsigned locations, CXSourceLocation *pLocations) {
  if (locations) {
...
_Use_decl_annotations_ HRESULT
DxcInclusion::Create(CXFile file, unsigned locations,
                     CXSourceLocation *pLocations, IDxcInclusion **pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  *pResult = nullptr;
...
_Use_decl_annotations_ HRESULT DxcInclusion::GetIncludedFile(
    _Outptr_result_nullonfailure_ IDxcFile **pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcFile::Create(m_file, pResult);
}

_Use_decl_annotations_ HRESULT
DxcInclusion::GetStackLength(_Out_ unsigned *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcInclusion::GetStackItem(
    unsigned index,
...
_Use_decl_annotations_ HRESULT
DxcIndex::Create(hlsl::DxcLangExtensionsHelper &langHelper, DxcIndex **index) {
  if (index == nullptr)
    return E_POINTER;
  *index = nullptr;
...
_Use_decl_annotations_ HRESULT DxcIndex::ParseTranslationUnit(
...
_Use_decl_annotations_ HRESULT DxcIntelliSense::CreateIndex(IDxcIndex **index) {
  DxcThreadMalloc TM(m_pMalloc);
  CComPtr<DxcIndex> local;
...
_Use_decl_annotations_ HRESULT
DxcIntelliSense::GetNullLocation(IDxcSourceLocation **location) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcSourceLocation::Create(clang_getNullLocation(), location);
}

_Use_decl_annotations_ HRESULT
DxcIntelliSense::GetNullRange(IDxcSourceRange **location) {
  DxcThreadMalloc TM(m_pMalloc);
  return DxcSourceRange::Create(clang_getNullRange(), location);
}

_Use_decl_annotations_ HRESULT
DxcIntelliSense::GetRange(IDxcSourceLocation *start, IDxcSourceLocation *end,
...
HRESULT DxcIntelliSense::GetDefaultEditingTUOptions(
    _Out_ DxcTranslationUnitFlags *pValue) {
  if (pValue == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcIntelliSense::CreateUnsavedFile(
    LPCSTR fileName, LPCSTR contents, unsigned contentLength,
...
_Use_decl_annotations_ HRESULT DxcSourceLocation::Create(
    const CXSourceLocation &location, IDxcSourceLocation **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
HRESULT DxcSourceLocation::IsEqualTo(_In_ IDxcSourceLocation *other,
                                     _Out_ BOOL *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  if (other == nullptr) {
...
HRESULT DxcSourceLocation::GetSpellingLocation(_Outptr_opt_ IDxcFile **pFile,
...
HRESULT DxcSourceLocation::GetPresumedLocation(_Outptr_opt_ LPSTR *pFilename,
                                               _Out_opt_ unsigned *pLine,
                                               _Out_opt_ unsigned *pCol) {
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT
DxcSourceRange::Create(const CXSourceRange &range, IDxcSourceRange **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
_Use_decl_annotations_ HRESULT DxcSourceRange::IsNull(BOOL *pValue) {
  if (pValue == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcSourceRange::GetStart(IDxcSourceLocation **pValue) {
...
_Use_decl_annotations_ HRESULT
DxcSourceRange::GetEnd(IDxcSourceLocation **pValue) {
  CXSourceLocation location = clang_getRangeEnd(m_range);
  DxcThreadMalloc TM(m_pMalloc);
  return DxcSourceLocation::Create(location, pValue);
}

HRESULT DxcSourceRange::GetOffsets(_Out_ unsigned *startOffset,
                                   _Out_ unsigned *endOffset) {
...
_Use_decl_annotations_ HRESULT DxcToken::Create(const CXTranslationUnit &tu,
                                                const CXToken &token,
                                                IDxcToken **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcToken::GetLocation(IDxcSourceLocation **pValue) {
  if (pValue == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT DxcToken::GetExtent(IDxcSourceRange **pValue) {
  if (pValue == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT DxcToken::GetSpelling(LPSTR *pValue) {
  if (pValue == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT
DxcTranslationUnit::GetCursor(IDxcCursor **pCursor) {
  DxcThreadMalloc TM(m_pMalloc);
  if (m_tu == nullptr)
    return E_FAIL;
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::Tokenize(
    IDxcSourceRange *range, IDxcToken ***pTokens, unsigned *pTokenCount) {
  if (range == nullptr)
    return E_INVALIDARG;
  if (pTokens == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcTranslationUnit::GetLocation(_In_ IDxcFile *file, unsigned line,
                                unsigned column, IDxcSourceLocation **pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  *pResult = nullptr;
...
_Use_decl_annotations_ HRESULT
DxcTranslationUnit::GetDiagnostic(unsigned index, IDxcDiagnostic **pValue) {
  if (pValue == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::GetFile(LPCSTR name,
                                                           IDxcFile **pResult) {
  if (name == nullptr)
    return E_INVALIDARG;
  if (pResult == nullptr)
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::GetFileName(LPSTR *pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return CXStringToAnsiAndDispose(clang_getTranslationUnitSpelling(m_tu),
                                  pResult);
}

_Use_decl_annotations_ HRESULT DxcTranslationUnit::Reparse(
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::GetCursorForLocation(
    IDxcSourceLocation *location, IDxcCursor **pResult) {
  if (location == nullptr)
    return E_INVALIDARG;
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::GetLocationForOffset(
    IDxcFile *file, unsigned offset, IDxcSourceLocation **pResult) {
  if (file == nullptr)
    return E_INVALIDARG;
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::GetSkippedRanges(
    IDxcFile *file, unsigned *pResultCount, IDxcSourceRange ***pResult) {
  if (file == nullptr)
    return E_INVALIDARG;
...
HRESULT DxcTranslationUnit::GetDiagnosticDetails(
    unsigned index, DxcDiagnosticDisplayOptions options,
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::GetInclusionList(
    unsigned *pResultCount, IDxcInclusion ***pResult) {
...
_Use_decl_annotations_ HRESULT DxcTranslationUnit::CodeCompleteAt(
...
_Use_decl_annotations_ HRESULT DxcType::Create(const CXType &type,
                                               IDxcType **pObject) {
  if (pObject == nullptr)
    return E_POINTER;
  *pObject = nullptr;
...
_Use_decl_annotations_ HRESULT DxcType::GetSpelling(LPSTR *pResult) {
  DxcThreadMalloc TM(m_pMalloc);
  return CXStringToAnsiAndDispose(clang_getTypeSpelling(m_type), pResult);
}

_Use_decl_annotations_ HRESULT DxcType::IsEqualTo(IDxcType *other,
                                                  BOOL *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  if (other == nullptr) {
...
_Use_decl_annotations_ HRESULT DxcType::GetKind(DxcTypeKind *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcCodeCompleteResults::GetNumResults(unsigned *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcCodeCompleteResults::GetResultAt(
    unsigned index, IDxcCompletionResult **pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcCompletionResult::GetCursorKind(DxcCursorKind *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT
DxcCompletionString::GetNumCompletionChunks(unsigned *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
...
_Use_decl_annotations_ HRESULT DxcCompletionString::GetCompletionChunkKind(
    unsigned chunkNumber, DxcCompletionChunkKind *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  *pResult =
...
_Use_decl_annotations_ HRESULT DxcCompletionString::GetCompletionChunkText(
    unsigned chunkNumber, LPSTR *pResult) {
  if (pResult == nullptr)
    return E_POINTER;
  DxcThreadMalloc TM(m_pMalloc);
