...
// CHECK: [[sampled_result1:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex1_load]] [[v2fc]] Bias|ConstOffset %float_0_5 [[v2ic]]
    float4 val1 = tex2d.SampleBias(float2(0.5, 0.25), 0.5f, int2(2, 3));
...
// CHECK: [[sampled_result2:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex2_load]] [[v2fc]] Bias|ConstOffset|MinLod %float_0_5 [[v2ic]] %float_2_5
    float4 val2 = tex2d.SampleBias(float2(0.5, 0.25), 0.5f, int2(2, 3), 2.5f);
...
// CHECK:                        OpStore %status [[status_0]]
    uint status;
    float4 val3 = tex2d.SampleBias(float2(0.5, 0.25), 0.5f, int2(2, 3), 2.5f, status);

// CHECK: [[load_arr1:%[a-zA-Z0-9_]+]] = OpLoad [[type_2d_sampled_image_array]] %tex2dArray
// CHECK: [[sampled_arr1:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[load_arr1]] [[v3fc]] Bias|ConstOffset %float_0_5 [[v2ic]]
    float4 val4 = tex2dArray.SampleBias(float3(0.5, 0.25, 0), 0.5f, int2(2, 3));
...
// CHECK: [[cube_bias:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[cube_load]] {{%[0-9]+}} Bias|MinLod %float_0_5 %float_1
    float4 val7 = texCube.SampleBias(float3(0.5, 0.25, 0), 0.5f, 1);

// CHECK: [[cube_arr_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_cube_sampled_image_array]] %texCubeArray
// CHECK: [[cube_arr_bias:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[cube_arr_load]] [[v4fc]] Bias %float_0_5
    float4 val8 = texCubeArray.SampleBias(float4(0.5, 0.25, 0, 0), 0.5f);
