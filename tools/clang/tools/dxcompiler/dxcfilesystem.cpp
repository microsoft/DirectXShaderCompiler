...
public:
  ...
  BOOL FindNextFileW(_In_ HANDLE hFindFile,
  ...
  BOOL SetFileTime(_In_ HANDLE hFile, _In_opt_ const FILETIME *lpCreationTime,
  ...
  BOOL GetFileInformationByHandle(_In_ HANDLE hFile,
                                  _Out_ LPBY_HANDLE_FILE_INFORMATION
                                      lpFileInformation) throw() override {
  ...
  BOOL CreateHardLinkW(_In_ LPCWSTR lpFileName,
                       _In_ LPCWSTR lpExistingFileName) throw() override {
    SetLastError(ERROR_NOT_CAPABLE);
    return FALSE;
  }
  BOOL MoveFileExW(_In_ LPCWSTR lpExistingFileName,
  ...
  _Success_(return != 0 && return < nBufferLength) DWORD
  ...
  _Success_(return != 0 && return < nSize) DWORD
  ...
  DWORD GetTempPathW(DWORD nBufferLength,
                     _Out_writes_to_opt_(nBufferLength, return +1)
                         LPWSTR lpBuffer) throw() override {
    SetLastError(ERROR_NOT_CAPABLE);
    return FALSE;
  }
  BOOLEAN CreateSymbolicLinkW(_In_ LPCWSTR lpSymlinkFileName,
  ...
  BOOL ReadFile(_In_ HANDLE hFile,
  ...
  HANDLE CreateFileMappingW(_In_ HANDLE hFile, _In_ DWORD flProtect,
                            _In_ DWORD dwMaximumSizeHigh,
                            _In_ DWORD dwMaximumSizeLow) throw() override {
    SetLastError(ERROR_NOT_CAPABLE);
    return INVALID_HANDLE_VALUE;
  }
  LPVOID MapViewOfFile(_In_ HANDLE hFileMappingObject,
                       _In_ DWORD dwDesiredAccess, _In_ DWORD dwFileOffsetHigh,
  ...
  int Read(int fd, _Out_bytecap_(count) void *buffer,
  ...
  int Write(int fd, _In_bytecount_(count) const void *buffer,
            unsigned int count) throw() override {
    CComPtr<IStream> stream;
    GetStreamForFD(fd, &stream);
    if (stream == nullptr) {
