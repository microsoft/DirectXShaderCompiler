...
// CHECK: [[tex3:%[a-zA-Z0-9_]+]] = OpVariable [[ptr_type_1]] UniformConstant

vk::SampledTexture2D tex1 : register(t0);
vk::SampledTexture2D<float4> tex2 : register(t1);
vk::SampledTexture2D<float2> tex3 : register(t2);
...
// CHECK: [[tex5_load:%[a-zA-Z0-9_]+]] = OpLoad [[type_sampled_image_1]] [[tex3]]
// CHECK: [[sampled_result5:%[a-zA-Z0-9_]+]] = OpImageSampleImplicitLod %v4float [[tex5_load]] [[v2fc]] None
// CHECK: [[val5:%[a-zA-Z0-9_]+]] = OpVectorShuffle %v2float [[sampled_result5]] [[sampled_result5]] 0 1
    float2 val5 = tex3.Sample(float2(0.5, 0.25));
    return 1.0;
}
