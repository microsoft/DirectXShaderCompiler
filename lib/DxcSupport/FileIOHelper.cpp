...
_Use_decl_annotations_ HRESULT ReadBinaryFile(IMalloc *pMalloc,
                                              LPCWSTR pFileName, void **ppData,
...
_Use_decl_annotations_ HRESULT ReadBinaryFile(LPCWSTR pFileName, void **ppData,
                                              DWORD *pDataSize) throw() {
  return ReadBinaryFile(GetGlobalHeapMalloc(), pFileName, ppData, pDataSize);
}

_Use_decl_annotations_ HRESULT WriteBinaryFile(LPCWSTR pFileName,
                                               const void *pData,
...
_Use_decl_annotations_ UINT32 DxcCodePageFromBytes(const char *bytes,
...
public:
  ...
  static HRESULT
  CreateFromBlob(_In_ IDxcBlob *pBlob, _In_ IMalloc *pMalloc,
  ...
  static HRESULT
  CreateFromMalloc(LPCVOID buffer, IMalloc *pIMalloc, SIZE_T bufferSize,
                   bool encodingKnown, UINT32 codePage,
                   _COM_Outptr_ InternalDxcBlobEncoding_Impl **pEncoding) {
  ...
  GetEncoding(_Out_ BOOL *pKnown, _Out_ UINT32 *pCodePage) override {
...
DxcCreateBlobFromBlob(IDxcBlob *pBlob, UINT32 offset, UINT32 length,
...
_Use_decl_annotations_ HRESULT
DxcCreateBlobFromFile(LPCWSTR pFileName, UINT32 *pCodePage,
                      IDxcBlobEncoding **ppBlobEncoding) throw() {
...
HRESULT DxcCreateBlobFromPinned(_In_bytecount_(size) LPCVOID pText, UINT32 size,
...
_Use_decl_annotations_ HRESULT DxcCreateBlobWithEncodingFromStream(
...
_Use_decl_annotations_ HRESULT DxcCreateBlobWithEncodingOnMalloc(
...
_Use_decl_annotations_ HRESULT DxcCreateBlobWithEncodingOnMallocCopy(
...
_Use_decl_annotations_ HRESULT DxcGetBlobAsWide(
    IDxcBlob *pBlob, IMalloc *pMalloc, IDxcBlobWide **pBlobEncoding) throw() {
  IFRBOOL(pBlob, E_POINTER);
  IFRBOOL(pBlobEncoding, E_POINTER);
  *pBlobEncoding = nullptr;
...
HRESULT
CreateMemoryStream(_In_ IMalloc *pMalloc,
                   _COM_Outptr_ AbstractMemoryStream **ppResult) throw() {
...
HRESULT CreateReadOnlyBlobStream(_In_ IDxcBlob *pSource,
                                 _COM_Outptr_ IStream **ppResult) throw() {
...
HRESULT CreateFixedSizeMemoryStream(
    _In_ LPBYTE pBuffer, size_t size,
    _COM_Outptr_ AbstractMemoryStream **ppResult) throw() {
