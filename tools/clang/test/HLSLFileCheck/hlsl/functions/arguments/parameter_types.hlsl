...
// CHECK: float %a, <4 x float> %b, %struct.T* %t, %class.matrix.float.2.3.Col %m, [3 x <2 x float>]* %n
// CHECK: float* noalias dereferenceable(4) %a, <4 x float>* noalias dereferenceable(16) %b, %struct.T* noalias %t, %class.matrix.float.2.3.Col* noalias dereferenceable(24) %m, [3 x <2 x float>]* noalias %n
