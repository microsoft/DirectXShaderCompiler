...
struct Mat
{
// CHECK:-FieldDecl {{.*}}, col:22> col:22 referenced m 'row_major matrix<float, 2, 2>'
...
// CHECK:|-FunctionDecl 0x{{.*}}:20 used foo 'row_major matrix<float, 3, 3> ()'
...
// CHECK:-FunctionDecl 0x{{.*}} main 'float4 (column_major matrix<float, 4, 4>)'
// CHECK-NEXT:|-ParmVarDecl 0x{{.*}} <col:13, col:35> col:35 used m2 'column_major matrix<float, 4, 4>'
