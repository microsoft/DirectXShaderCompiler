...
public:
  ...
  virtual BOOL GetFileInformationByHandle(_In_ HANDLE hFile,
                                          _Out_ LPBY_HANDLE_FILE_INFORMATION
                                              lpFileInformation) throw() = 0;
  virtual DWORD GetFileType(_In_ HANDLE hFile) throw() = 0;
  virtual BOOL CreateHardLinkW(_In_ LPCWSTR lpFileName,
                               _In_ LPCWSTR lpExistingFileName) throw() = 0;
  virtual BOOL MoveFileExW(_In_ LPCWSTR lpExistingFileName,
                           _In_opt_ LPCWSTR lpNewFileName,
                           _In_ DWORD dwFlags) throw() = 0;
  ...
  virtual BOOL ReadFile(_In_ HANDLE hFile,
                        ...
                        _Out_opt_ LPDWORD lpNumberOfBytesRead) throw() = 0;
  virtual HANDLE CreateFileMappingW(_In_ HANDLE hFile, _In_ DWORD flProtect,
                                    _In_ DWORD dwMaximumSizeHigh,
                                    _In_ DWORD dwMaximumSizeLow) throw() = 0;
  virtual LPVOID MapViewOfFile(_In_ HANDLE hFileMappingObject,
  ...
  virtual errno_t
  resize_file(_In_ LPCWSTR path,
              uint64_t size) throw() = 0; // A number of C calls.
  virtual int Read(int fd, _Out_bytecap_(count) void *buffer,
                   unsigned int count) throw() = 0;
