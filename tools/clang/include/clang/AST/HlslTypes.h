...
struct MatrixMemberAccessPositions {
  ...
  void GetPosition(uint32_t index, _Out_ uint32_t *row,
                   _Out_ uint32_t *col) const {
    assert(index < 4);
    switch (index) {
    case 0:
      *row = R0_Row;
...
clang::CXXRecordDecl *DeclareTemplateTypeWithHandle(
    clang::ASTContext &context, llvm::StringRef name, uint8_t templateArgCount,
...
clang::CXXRecordDecl *
DeclareUIntTemplatedTypeWithHandle(clang::ASTContext &context,
...
clang::CXXMethodDecl *CreateObjectFunctionDeclarationWithParams(
    clang::ASTContext &context, _In_ clang::CXXRecordDecl *recordDecl,
...
_Success_(return != false) bool TryParseMatrixShorthand(
    _In_count_(typeNameLen) const char *typeName, size_t typeNameLen,
    _Out_ HLSLScalarType *parsedType, _Out_ int *rowCount, _Out_ int *colCount,
    _In_ const clang::LangOptions &langOption);

_Success_(return != false) bool TryParseVectorShorthand(
    _In_count_(typeNameLen) const char *typeName, size_t typeNameLen,
    _Out_ HLSLScalarType *parsedType, _Out_ int *elementCount,
    _In_ const clang::LangOptions &langOption);

_Success_(return != false) bool TryParseScalar(
    _In_count_(typenameLen) const char *typeName, size_t typeNameLen,
    _Out_ HLSLScalarType *parsedType,
    _In_ const clang::LangOptions &langOption);

_Success_(return != false) bool TryParseAny(
    _In_count_(typenameLen) const char *typeName, size_t typeNameLen,
    _Out_ HLSLScalarType *parsedType, _Out_ int *rowCount, _Out_ int *colCount,
    _In_ const clang::LangOptions &langOption);

_Success_(return != false) bool TryParseString(
    _In_count_(typenameLen) const char *typeName, size_t typeNameLen,
    _In_ const clang::LangOptions &langOptions);

_Success_(return != false) bool TryParseMatrixOrVectorDimension(
    _In_count_(typeNameLen) const char *typeName, size_t typeNameLen,
