...
static const ArTypeObjectKind g_ObjectTT[] = {AR_TOBJ_OBJECT, AR_TOBJ_UNKNOWN};
...
AddHLSLIntrinsicFunction(ASTContext &context, _In_ NamespaceDecl *NS,
  ...
  if (IsAtomicOperation(tableName, static_cast<IntrinsicOp>(pIntrinsic->Op))) {
    ...
    functionArgQualTypes[kAtomicDstOperandIdx] = context.getLValueReferenceType(
...
public:
  ...
  bool IsValidTemplateArgumentType(SourceLocation argLoc, const QualType &type,
    ...
    // TemplateTypeParm here will be construction of vector return template in
    // matrix operator[]
...
#define CAB(cond, arg)                                                         \
  ...
  for (size_t i = 0; i <= Args.size(); i++) {
    ...
    //  argTypes[i] =
    //  m_context->getLValueReferenceType(argTypes[i].withConst());
    //}
  }
...
void hlsl::HandleDeclAttributeForHLSL(Sema &S, Decl *D, const AttributeList &A,
  ...
  case AttributeList::AT_HLSLColumnMajor:
    declAttr = ::new (S.Context) HLSLColumnMajorAttr(
        A.getRange(), S.Context, A.getAttributeSpellingListIndex());
    break;
  case AttributeList::AT_HLSLRowMajor:
    declAttr = ::new (S.Context) HLSLRowMajorAttr(
        A.getRange(), S.Context, A.getAttributeSpellingListIndex());
    break;
...
Decl *Sema::ActOnHLSLBufferView(Scope *bufferScope, SourceLocation KwLoc,
  ...
  // Prevent array type in template.  The only way to specify an array in the
  // template type is to use a typedef, so we will strip non-typedef arrays off,
  // since these are the legal array dimensions for the CBV/TBV, and if any
  // array type remains, that is illegal.
  QualType declType = cast<VarDecl>(namedDecl)->getType();
  while (declType->isArrayType() &&
         declType->getTypeClass() != Type::TypeClass::Typedef) {
  ...
  DeclContext *lexicalParent = getCurLexicalContext();
  clang::HLSLBufferDecl *result = HLSLBufferDecl::Create(
      Context, lexicalParent, iscbuf, /*isConstantBufferView*/ true, KwLoc,
      Ident, Loc, hlslAttrs, Loc);
...
Sema::getHLSLDefaultSpecialization(ClassTemplateDecl *Decl) {
  if (Decl->getTemplateParameters()->getMinRequiredArguments() == 0) {
    ...
    if (!CheckTemplateArgumentList(Decl, Decl->getSourceRange().getEnd(),
                                   EmptyArgs, false, Converted)) {
