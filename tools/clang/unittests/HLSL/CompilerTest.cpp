...
#if _WIN32 // API -setrootsignature requires reflection, which isn't supported
           // on non-win
...
#endif // _WIN32 - API -setrootsignature requires reflection, which isn't
       // supported on non-win
...
public:
  ...
  virtual void *STDMETHODCALLTYPE Realloc(_In_opt_ void *pv,
  ...
  virtual int STDMETHODCALLTYPE DidAlloc(_In_opt_ void *pv) override {
    return -1; // don't know
  }
...
#if defined(_WIN32) &&                                                         \
    !(defined(_M_ARM64) ||                                                     \
      defined(_M_ARM64EC)) // this test has issues on ARM64; disable until we
