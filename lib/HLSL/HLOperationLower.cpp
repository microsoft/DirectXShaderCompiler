...
struct ResLoadHelper {
  ...
  ResLoadHelper(Instruction *Inst, DxilResource::Kind RK, Value *h, Value *idx,
      ...
      : intrinsicOpCode(IntrinsicOp::Num_Intrinsics), handle(h), retVal(Inst),
        addr(idx), offset(Offset), status(status), mipLevel(mip), customAlignment(0) {
  ...
  Value *status;
  Value *mipLevel;
  unsigned customAlignment; // For AlignedLoad/AlignedStore - 0 means use default
};
...
ResLoadHelper::ResLoadHelper(CallInst *CI, DxilResource::Kind RK,
    ...
    : intrinsicOpCode(IOP), handle(hdl), offset(nullptr), status(nullptr), customAlignment(0) {
  opcode = LoadOpFromResKind(RK);
  ...
  if (IOP == IntrinsicOp::MOP_AlignedLoad) {
    ...
    if (argc > kAlignmentIdx) {
      if (ConstantInt *AlignConst = dyn_cast<ConstantInt>(CI->getArgOperand(kAlignmentIdx))) {
...
static SmallVector<Value *, 10> GetBufLoadArgs(ResLoadHelper helper,
  ...
  // Use custom alignment if provided (for AlignedLoad), otherwise calculate default
  unsigned alignment;
...
void TranslateStore(DxilResource::Kind RK, Value *handle, Value *val,
                    ...
                    hlsl::OP *OP, Value *sampIdx = nullptr, unsigned customAlignment = 0) {
  Type *Ty = val->getType();
  OP::OpCode opcode = OP::OpCode::NumOpCodes;
  bool IsTyped = true;
  switch (RK) {
  ...
  // Use custom alignment if provided (for AlignedStore), otherwise calculate default
  unsigned alignValue;
...
Value *TranslateResourceStore(CallInst *CI, IntrinsicOp IOP, OP::OpCode opcode,
  ...
  if (IOP == IntrinsicOp::MOP_AlignedStore) {
    ...
    valueArgIdx = kAlignmentIdx + 1; // Value is after alignment for AlignedStore
  }
  ...
  TranslateStore(RK, handle, val, offset, UndefI, Builder, hlslOP, nullptr, customAlignment);
...
IntrinsicLower gLowerTable[] = {
    ...
    {IntrinsicOp::MOP_AlignedLoad, TranslateResourceLoad, DXIL::OpCode::NumOpCodes},
    {IntrinsicOp::MOP_AlignedStore, TranslateResourceStore, DXIL::OpCode::NumOpCodes},
};
} // namespace
static_assert(
