...
public:
  ...
  static HRESULT Create(const CXCursor &cursor,
                        _Outptr_result_nullonfailure_ IDxcCursor **pObject);

  HRESULT STDMETHODCALLTYPE
  GetExtent(_Outptr_result_nullonfailure_ IDxcSourceRange **pRange) override;
  ...
  HRESULT STDMETHODCALLTYPE
  GetKindFlags(_Out_ DxcCursorKindFlags *pResult) override;
  HRESULT STDMETHODCALLTYPE GetSemanticParent(
      _Outptr_result_nullonfailure_ IDxcCursor **pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetLexicalParent(_Outptr_result_nullonfailure_ IDxcCursor **pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetCursorType(_Outptr_result_nullonfailure_ IDxcType **pResult) override;
  HRESULT STDMETHODCALLTYPE GetNumArguments(_Out_ int *pResult) override;
  HRESULT STDMETHODCALLTYPE GetArgumentAt(
      int index, _Outptr_result_nullonfailure_ IDxcCursor **pResult) override;
  HRESULT STDMETHODCALLTYPE GetReferencedCursor(
      _Outptr_result_nullonfailure_ IDxcCursor **pResult) override;
  HRESULT STDMETHODCALLTYPE GetDefinitionCursor(
      _Outptr_result_nullonfailure_ IDxcCursor **pResult) override;
  HRESULT STDMETHODCALLTYPE
  ...
  HRESULT STDMETHODCALLTYPE
  GetSpelling(_Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE IsEqualTo(_In_ IDxcCursor *other,
  ...
  HRESULT STDMETHODCALLTYPE
  GetDisplayName(_Outptr_result_maybenull_ BSTR *pResult) override;
  /// <summary>Gets the qualified name for the symbol the cursor refers
  /// to.</summary>
  HRESULT STDMETHODCALLTYPE
  GetQualifiedName(BOOL includeTemplateArgs,
                   _Outptr_result_maybenull_ BSTR *pResult) override;
  ...
  GetSnappedChild(_In_ IDxcSourceLocation *location,
                  _Outptr_result_maybenull_ IDxcCursor **pResult) override;
};
...
public:
  ...
  static HRESULT Create(const CXDiagnostic &diagnostic,
                        _Outptr_result_nullonfailure_ IDxcDiagnostic **pObject);

  HRESULT STDMETHODCALLTYPE
  FormatDiagnostic(DxcDiagnosticDisplayOptions options,
                   _Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetSeverity(_Out_ DxcDiagnosticSeverity *pResult) override;
  HRESULT STDMETHODCALLTYPE GetLocation(
      _Outptr_result_nullonfailure_ IDxcSourceLocation **pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetSpelling(_Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetCategoryText(_Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE GetNumRanges(_Out_ unsigned *pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetRangeAt(unsigned index,
             _Outptr_result_nullonfailure_ IDxcSourceRange **pResult) override;
  ...
  GetFixItAt(unsigned index,
             ...
             _Outptr_result_maybenull_ LPSTR *pText) override;
};
...
public:
  ...
  static HRESULT Create(const CXFile &file,
                        _Outptr_result_nullonfailure_ IDxcFile **pObject);

  const CXFile &GetFile() const { return m_file; }
  HRESULT STDMETHODCALLTYPE
  GetName(_Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE IsEqualTo(_In_ IDxcFile *other,
                                      _Out_ BOOL *pResult) override;
};
...
private:
  DXC_MICROCOM_TM_REF_FIELDS()
  CXFile m_file;
  _Field_size_(m_locationLength) CXSourceLocation *m_locations;
...
public:
  ...
  HRESULT Initialize(CXFile file, unsigned locations,
                     _In_count_(locations) CXSourceLocation *pLocation);
  static HRESULT Create(CXFile file, unsigned locations,
                        _In_count_(locations) CXSourceLocation *pLocation,
                        _COM_Outptr_ IDxcInclusion **pResult);

  HRESULT STDMETHODCALLTYPE
  GetIncludedFile(_Outptr_result_nullonfailure_ IDxcFile **pResult) override;
  HRESULT STDMETHODCALLTYPE GetStackLength(_Out_ unsigned *pResult) override;
  HRESULT STDMETHODCALLTYPE GetStackItem(
      unsigned index,
      _Outptr_result_nullonfailure_ IDxcSourceLocation **pResult) override;
};
...
public:
  ...
  static HRESULT Create(hlsl::DxcLangExtensionsHelper &langHelper,
                        _Outptr_result_nullonfailure_ DxcIndex **index);

  HRESULT STDMETHODCALLTYPE SetGlobalOptions(DxcGlobalOptions options) override;
  HRESULT STDMETHODCALLTYPE
  GetGlobalOptions(_Out_ DxcGlobalOptions *options) override;
...
public:
  ...
  HRESULT STDMETHODCALLTYPE
  CreateIndex(_Outptr_result_nullonfailure_ IDxcIndex **index) override;
  HRESULT STDMETHODCALLTYPE GetNullLocation(
      _Outptr_result_nullonfailure_ IDxcSourceLocation **location) override;
  HRESULT STDMETHODCALLTYPE GetNullRange(
  ...
  HRESULT STDMETHODCALLTYPE
  GetDefaultEditingTUOptions(_Out_ DxcTranslationUnitFlags *pValue) override;
...
public:
  ...
  static HRESULT
  Create(const CXSourceLocation &location,
         _Outptr_result_nullonfailure_ IDxcSourceLocation **pObject);

  const CXSourceLocation &GetLocation() const { return m_location; }
  HRESULT STDMETHODCALLTYPE IsEqualTo(_In_ IDxcSourceLocation *other,
                                      _Out_ BOOL *pValue) override;
  HRESULT STDMETHODCALLTYPE GetSpellingLocation(
      _Outptr_opt_ IDxcFile **pFile, _Out_opt_ unsigned *pLine,
      _Out_opt_ unsigned *pCol, _Out_opt_ unsigned *pOffset) override;
  HRESULT STDMETHODCALLTYPE IsNull(_Out_ BOOL *pResult) override;
  HRESULT STDMETHODCALLTYPE
...
public:
  ...
  static HRESULT
  Create(const CXSourceRange &range,
         _Outptr_result_nullonfailure_ IDxcSourceRange **pObject);

  const CXSourceRange &GetRange() const { return m_range; }
  HRESULT STDMETHODCALLTYPE IsNull(_Out_ BOOL *pValue) override;
  HRESULT STDMETHODCALLTYPE
  GetStart(_Outptr_result_nullonfailure_ IDxcSourceLocation **pValue) override;
  HRESULT STDMETHODCALLTYPE
  GetEnd(_Outptr_result_nullonfailure_ IDxcSourceLocation **pValue) override;
  HRESULT STDMETHODCALLTYPE GetOffsets(_Out_ unsigned *startOffset,
                                       _Out_ unsigned *endOffset) override;
};
...
public:
  ...
  static HRESULT Create(const CXTranslationUnit &tu, const CXToken &token,
                        _Outptr_result_nullonfailure_ IDxcToken **pObject);

  HRESULT STDMETHODCALLTYPE GetKind(_Out_ DxcTokenKind *pValue) override;
  HRESULT STDMETHODCALLTYPE GetLocation(
      _Outptr_result_nullonfailure_ IDxcSourceLocation **pValue) override;
  HRESULT STDMETHODCALLTYPE
  GetExtent(_Outptr_result_nullonfailure_ IDxcSourceRange **pValue) override;
  HRESULT STDMETHODCALLTYPE
  GetSpelling(_Outptr_result_maybenull_ LPSTR *pValue) override;
};
...
public:
  ...
  HRESULT STDMETHODCALLTYPE GetLocation(
      _In_ IDxcFile *file, unsigned line, unsigned column,
      _Outptr_result_nullonfailure_ IDxcSourceLocation **pResult) override;
  HRESULT STDMETHODCALLTYPE GetNumDiagnostics(_Out_ unsigned *pValue) override;
  HRESULT STDMETHODCALLTYPE
  GetDiagnostic(unsigned index,
                _Outptr_result_nullonfailure_ IDxcDiagnostic **pValue) override;
  HRESULT STDMETHODCALLTYPE
  GetFile(_In_ const char *name,
          _Outptr_result_nullonfailure_ IDxcFile **pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetFileName(_Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE Reparse(_In_count_(num_unsaved_files)
                                        IDxcUnsavedFile **unsaved_files,
                                    unsigned num_unsaved_files) override;
  HRESULT STDMETHODCALLTYPE GetCursorForLocation(
      _In_ IDxcSourceLocation *location,
      _Outptr_result_nullonfailure_ IDxcCursor **pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetLocationForOffset(_In_ IDxcFile *file, unsigned offset,
                       _Outptr_ IDxcSourceLocation **pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetSkippedRanges(_In_ IDxcFile *file, _Out_ unsigned *pResultCount,
                   _Outptr_result_buffer_(*pResultCount)
  ...
  HRESULT STDMETHODCALLTYPE GetInclusionList(
      ...
      _Outptr_result_buffer_(*pResultCount) IDxcInclusion ***pResult) override;
  HRESULT STDMETHODCALLTYPE CodeCompleteAt(
      ...
      _Outptr_result_nullonfailure_ IDxcCodeCompleteResults **pResult) override;
};
...
public:
  ...
  static HRESULT Create(const CXType &type,
                        _Outptr_result_nullonfailure_ IDxcType **pObject);

  HRESULT STDMETHODCALLTYPE
  GetSpelling(_Outptr_result_maybenull_ LPSTR *pResult) override;
  HRESULT STDMETHODCALLTYPE IsEqualTo(_In_ IDxcType *other,
                                      _Out_ BOOL *pResult) override;
...
public:
  ...
  HRESULT STDMETHODCALLTYPE GetResultAt(
      unsigned index,
      _Outptr_result_nullonfailure_ IDxcCompletionResult **pResult) override;
};
...
public:
  ...
  HRESULT STDMETHODCALLTYPE
  GetCursorKind(_Out_ DxcCursorKind *pResult) override;
  HRESULT STDMETHODCALLTYPE GetCompletionString(
      _Outptr_result_nullonfailure_ IDxcCompletionString **pResult) override;
};
...
public:
  ...
  HRESULT STDMETHODCALLTYPE
  GetNumCompletionChunks(_Out_ unsigned *pResult) override;
  HRESULT STDMETHODCALLTYPE GetCompletionChunkKind(
      unsigned chunkNumber, _Out_ DxcCompletionChunkKind *pResult) override;
  HRESULT STDMETHODCALLTYPE
  GetCompletionChunkText(unsigned chunkNumber, _Out_ LPSTR *pResult) override;
};
