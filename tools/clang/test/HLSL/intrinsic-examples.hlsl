...
float4 RWByteAddressBufferMain(uint2 a : A, uint2 b : B) : SV_Target
{
  ...
  uav1.LoadHalf(h.x, status);                               /* expected-error {{HalfLoad/Store not allowed in min precision mode}} */
  uav1.LoadHalf2(h.x);                                      /* expected-error {{HalfLoad/Store not allowed in min precision mode}} expected-warning {{ignoring return value of function that only reads data}} */
  uav1.StoreHalf3(4, h.xyz);                                /* expected-error {{HalfLoad/Store not allowed in min precision mode}} */
  uav1.StoreHalf4(8, h);                                    /* expected-error {{HalfLoad/Store not allowed in min precision mode}} */
  return r;
}
