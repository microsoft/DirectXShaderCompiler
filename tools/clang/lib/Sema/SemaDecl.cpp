    ...
    if (Corrected.isKeyword()) {
      ...
      SuggestedType =
          getTypeName(*Corrected.getCorrectionAsIdentifierInfo(), IILoc, S,
                      tmpSS.isSet() ? &tmpSS : SS, false, false, ParsedType(),
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (isTemplateName(S, SS ? *SS : EmptySS, /*hasTemplateKeyword=*/false,
      ...
      if (TemplateDecl *TplDecl = TplName.getAsTemplateDecl()) {
        if (TplDecl->getLocation().isValid()) { // HLSL Change - ellide location
                                                // notes for built-ins
...
/// entity.
static void filterNonConflictingPreviousDecls(Sema &S, NamedDecl *decl,
...
static bool mergeDeclAttribute(Sema &S, NamedDecl *D,
  ...
  if (const auto *AA = dyn_cast<AvailabilityAttr>(Attr))
    NewAttr = S.mergeAvailabilityAttr(
        D, AA->getRange(), AA->getPlatform(), AA->getIntroduced(),
        AA->getDeprecated(), AA->getObsoleted(), AA->getUnavailable(),
        AA->getMessage(), Override, AttrSpellingListIndex);
...
#if 1 // HLSL Change Starts - commenting this out rather than fixing for inout
      // semantics - N/A for HLSL
...
#if 0  // HLSL Change Starts - commenting this out rather than fixing for inout
       // semantics - N/A for HLSL
...
/// is attached.
void Sema::MergeVarDeclTypes(
    VarDecl *New, VarDecl *Old, bool MergeTypeWithOld,
    ShadowMergeState &MergeState) { // HLSL Change - add merge state
...
///
void Sema::MergeVarDecl(
    VarDecl *New, LookupResult &Previous,
    ShadowMergeState &MergeState) { // HLSL Change - add merge state
  ...
  if (MostRecent != Old) {
    MergeVarDeclTypes(New, MostRecent,
                      ...
                      MergeState); // HLSL Change - add MergeState
    if (New->isInvalidDecl())
      return;
  }
  ...
  if (MergeState != ShadowMergeState_Effective) { // HLSL Change - avoid merging
                                                  // when shadowing
    // Merge "used" flag.
    if (Old->getMostRecentDecl()->isUsed(false))
      New->setIsUsed();
...
InjectAnonymousStructOrUnionMembers(Sema &SemaRef, Scope *S, DeclContext *Owner,
                                    ...
                                    bool MSAnonStruct) {
  unsigned diagKind = AnonRecord->isUnion()
                          ? diag::err_anonymous_union_member_redecl
    ...
    for (auto *Mem : Record->decls()) {
      if (auto *FD = dyn_cast<FieldDecl>(Mem)) {
        ...
        if (getLangOpts().MicrosoftExt &&
          ...
          Diag(Mem->getLocation(), DK) << (int)Record->isUnion();
          Invalid = true;
        }
      }
    }
...
NamedDecl *Sema::HandleDeclarator(Scope *S, Declarator &D,
  ...
  // we find one that is.
  ShadowMergeState MergeState =
      S->isForDeclScope() ? ShadowMergeState_Possible
                          : ShadowMergeState_Disallowed; // HLSL Change - check
                                                         // on original scope
  while ((S->getFlags() & Scope::DeclScope) == 0 ||
  ...
  if (getLangOpts().HLSL) {
    ...
    if (!DiagnoseHLSLDecl(D, DC, /*BitWidth*/ nullptr, TInfo,
                          IsParameterFalse)) {
  ...
  if (D.getDeclSpec().getStorageClassSpec() == DeclSpec::SCS_typedef) {
    ...
    New = ActOnVariableDeclarator(S, D, DC, TInfo, Previous, TemplateParamLists,
                                  AddToScope,
                                  MergeState); // HLSL Change - add merge state
  }
  ...
  if (!getLangOpts().CPlusPlus) {
    ...
    if (!IsVariableTemplateSpecialization)
      D.setRedeclaration(CheckVariableDeclaration(
          NewVD, Previous, MergeState)); // HLSL Change - add merge state
...
void Sema::CheckVariableDeclarationType(VarDecl *NewVD) {
  ...
  if (getLangOpts().OpenCL && NewVD->isFileVarDecl() &&
      T.getAddressSpace() != LangAS::opencl_constant && !T->isSamplerT()) {
  ...
  // scope.
  if ((getLangOpts().OpenCLVersion >= 120) && NewVD->isStaticLocal()) {
...
bool Sema::CheckVariableDeclaration(
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (TemplateParameterList *TemplateParams =
      ...
      if (TemplateParams->size() > 0) {
        ...
        if (TemplateParamLists.size() > 0)
          NewFD->setTemplateParameterListsInfo(
              Context, TemplateParamLists.size(), TemplateParamLists.data());
      ...
      if (TemplateParamLists.size() > 0)
        // For source fidelity, store all the template param lists.
        NewFD->setTemplateParameterListsInfo(Context, TemplateParamLists.size(),
                                             TemplateParamLists.data());
    }
    ...
    if (getLangOpts().CPlusPlus14 &&
      ...
      NewFD->setType(Context.getFunctionType(
          Result, FPT->getParamTypes(), FPT->getExtProtoInfo(),
          FPT->getParamMods())); // HLSL Change - add param mods
    }
  ...
  if (!getLangOpts().CPlusPlus) {
    ...
    if (!isDependentClassScopeExplicitSpecialization) {
      if (!getLangOpts().HLSL && !NewFD->isInvalidDecl() &&
          NewFD->isMain()) // HLSL Change: no main() checks in HLSL
        CheckMain(NewFD, D.getDeclSpec());

      if (!getLangOpts().HLSL && !NewFD->isInvalidDecl() &&
          NewFD->isMSVCRTEntryPoint()) // HLSL Change: no entry point checks in
                                       // HLSL
        CheckMSVCRTEntryPoint(NewFD);
  ...
  if (NewFD->hasAttr<OverloadableAttr>() &&
    ...
    QualType R =
        Context.getFunctionType(FT->getReturnType(), None, EPI,
                                None); // HLSL Change - all defaults are in
    NewFD->setType(R);
  }
...
void Sema::AddInitializerToDecl(Decl *RealDecl, Expr *Init, bool DirectInit,
  ...
  if (TypeMayContainAuto && VDecl->getType()->isUndeducedType()) {
    ...
    assert(!getLangOpts().HLSL && "auto types are not supported - merge type "
                                  "below is inconsequential"); // HLSL Change
    if (VarDecl *Old = VDecl->getPreviousDecl()) {
      // We never need to merge the type, because we cannot form an incomplete
      // array of auto, nor deduce such a type.
      ShadowMergeState MergeState =
          ShadowMergeState_Disallowed; // HLSL Change - add merge state
    ...
    if (DirectInit && !CXXDirectInit && isa<InitListExpr>(Init)) {
      Diag(Init->getLocStart(), diag::warn_auto_var_direct_list_init)
  ...
  if (!VDecl->isInvalidDecl()) {
    ...
    assert(Init != nullptr &&
           "otherwise result should be invalid"); // HLSL Change
  }
...
void Sema::CheckCompleteVariableDeclaration(VarDecl *var) {
  ...
  if (getLangOpts().ObjCAutoRefCount && var->hasLocalStorage()) {
  ...
  if (GlobalStorage && var->isThisDeclarationADefinition() &&
    ...
    if (var->getType().isConstQualified()
        // HLSL Change: for HLSL, const initialized isn't the same, unless
        // static
        && (!getLangOpts().HLSL || var->getTLSKind() == VarDecl::TLS_Static))
      Stack = &ConstSegStack;
    else if (!var->getInit()) {
      Stack = &BSSSegStack;
...
Decl *Sema::ActOnParamDeclarator(Scope *S, Declarator &D) {
  ...
  ParmVarDecl *New =
      CheckParameter(Context.getTranslationUnitDecl(), D.getLocStart(),
                     ...
                     hlsl::ParamModFromAttributeList(
                         DS.getAttributes().getList())); // HLSL Change
...
ParmVarDecl *Sema::CheckParameter(
    DeclContext *DC, SourceLocation StartLoc, SourceLocation NameLoc,
    IdentifierInfo *Name, QualType T, TypeSourceInfo *TSInfo, StorageClass SC,
    hlsl::ParameterModifier ParamMod) { // HLSL Change - add param mod
...
void Sema::CheckForFunctionRedefinition(
    FunctionDecl *FD, const FunctionDecl *EffectiveDefinition) {
  ...
  if (Name && Previous.empty() && (TUK == TUK_Reference || TUK == TUK_Friend)) {
    ...
    if (TUK == TUK_Reference) {
      ...
      while (S->isClassScope() ||
             (getLangOpts().CPlusPlus && S->isFunctionPrototypeScope()) ||
    ...
    if (TagDecl *PrevTagDecl = dyn_cast<TagDecl>(PrevDecl)) {
      ...
      if (TUK == TUK_Reference || TUK == TUK_Friend ||
        ...
        if (!Invalid) {
          ...
          if (!Attr &&
              ((TUK == TUK_Reference && (!PrevTagDecl->getFriendObjectKind() ||
                                         getLangOpts().MicrosoftExt)) ||
               TUK == TUK_Friend))
            return PrevTagDecl;
          ...
          if (getSourceManager().isInSystemHeader(Loc)) {
            if (!FD->hasAttr<UnavailableAttr>())
              FD->addAttr(UnavailableAttr::CreateImplicit(
                  Context, "this system field has retaining ownership", Loc));
            return false;
          }
        }
...
Sema::SkipBodyInfo Sema::shouldSkipAnonEnumBody(Scope *S, IdentifierInfo *II,
  ...
  NamedDecl *Hidden;
  if (PrevECD && !hasVisibleDefinition(
                     cast<NamedDecl>(PrevECD->getDeclContext()), &Hidden)) {
    SkipBodyInfo Skip;
...
void Sema::ActOnEnumBody(SourceLocation EnumLoc, SourceLocation LBraceLoc,
                         ...
                         ArrayRef<Decl *> Elements, Scope *S,
                         AttributeList *Attr) {
...
DeclResult Sema::ActOnModuleImport(SourceLocation AtLoc,
  ...
  ImportDecl *Import = ImportDecl::Create(
      Context, Context.getTranslationUnitDecl(),
      AtLoc.isValid() ? AtLoc : ImportLoc, Mod, IdentifierLocs);
