...
getMemBufferFromBlob(_In_ IDxcBlob *pBlob, const llvm::Twine &BufferName) {
...
std::unique_ptr<llvm::MemoryBuffer>
getMemBufferFromStream(_In_ IStream *pStream, const llvm::Twine &BufferName) {
...
STDMETHODIMP
dxil_dia::DataSource::loadDataFromIStream(_In_ IStream *pInputIStream) {
  try {
...
STDMETHODIMP
dxil_dia::DataSource::openSession(_COM_Outptr_ IDiaSession **ppSession) {
  DxcThreadMalloc TM(m_pMalloc);
  *ppSession = nullptr;
  if (m_module.get() == nullptr)
    return E_FAIL;
