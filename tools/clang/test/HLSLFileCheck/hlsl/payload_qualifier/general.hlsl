...
// RUN: %dxc -T lib_6_5 -D TEST_NUM=3 %s -allow-payload-qualifiers | FileCheck -input=stderr -check-prefix=CHK3 %s
// RUN: %dxc -T lib_6_6 -D TEST_NUM=3 %s | FileCheck -input=stderr -check-prefix=CHK4 %s
// RUN: %dxc -T lib_6_6 -D TEST_NUM=3 %s -allow-payload-qualifiers | FileCheck -check-prefix=CHK5 %s
// RUN: %dxc -T lib_6_6 -D TEST_NUM=4 %s -allow-payload-qualifiers | FileCheck -check-prefix=CHK6 %s
...
// CHK3: Invalid target for payload access qualifiers. Only lib_6_6 and beyond are supported.
// CHK4: warning: payload access qualifieres are only support for target lib_6_6 and beyond. You can opt-in for lib_6_6 with the -allow-payload-qualifiers flag. Qualifiers will be dropped.
