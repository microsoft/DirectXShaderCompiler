...
NamedDecl *Parser::ParseCXXInlineMethodDef(AccessSpecifier AS,
  ...
  if (TryConsumeToken(tok::equal)) {
    ...
    // HLSL Change Starts
    if (getLangOpts().HLSL) {
      Diag(Tok, diag::err_hlsl_unsupported_construct)
        << "function deletion and defaulting";
      SkipUntil(tok::semi);
      return FnD;
    }
    // HLSL Change Ends
...
void Parser::ParseLexedMethodDef(LexedMethod &LM) {
  ...
  if (Tok.is(tok::kw_try)) {
    // HLSL Change Starts - remove support for function try
    if (getLangOpts().HLSL) {
  ...
  if (Tok.is(tok::colon)) {
    ...
    // Error recovery.
    if (!Tok.is(tok::l_brace) || getLangOpts().HLSL) { // HLSL Change
      FnScope.Exit();
      Actions.ActOnFinishFunctionBody(LM.D, nullptr);
