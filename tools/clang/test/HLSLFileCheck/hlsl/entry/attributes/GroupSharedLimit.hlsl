...
[GroupSharedLimit(NUM_BYTES_OF_SHARED_MEM)]
    [numthreads(THREAD_GROUP_SIZE_X, 1, 1)] void
    MainPass(uint3 DTid : SV_DispatchThreadID) {
...
[GroupSharedLimit(NUM_BYTES_OF_SHARED_MEM)]
    [numthreads(THREAD_GROUP_SIZE_X, 1, 1)] void
    MainFail(uint3 DTid : SV_DispatchThreadID) {
      uint iterations = NUM_DWORDS_SHARED_MEM / THREAD_GROUP_SIZE_X;
      ...
      for (uint i = 0; i < iterations; i++) {
        ...
        [numthreads(THREAD_GROUP_SIZE_X, 1, 1)] void MainFail2(uint3 DTid : SV_DispatchThreadID) {
  uint iterations = NUM_DWORDS_SHARED_MEM / THREAD_GROUP_SIZE_X;
