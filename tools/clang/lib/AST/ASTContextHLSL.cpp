...
CXXRecordDecl *hlsl::DeclareTemplateTypeWithHandleInDeclContext(
    ...
    uint8_t templateArgCount, _In_opt_ TypeSourceInfo *defaultTypeArgValue) {
...
hlsl::CreateFunctionTemplateDecl(ASTContext &context,
                                 _In_ CXXRecordDecl *recordDecl,
                                 _In_ CXXMethodDecl *functionDecl,
                                 _In_count_(templateParamNamedDeclsCount)
                                     NamedDecl **templateParamNamedDecls,
...
static void AssociateParametersToFunctionPrototype(
    _In_ TypeSourceInfo *tinfo,
    _In_count_(numParams) ParmVarDecl **paramVarDecls, unsigned int numParams) {
...
static bool TryParseColOrRowChar(const char digit, _Out_ int *count) {
  if ('1' <= digit && digit <= '4') {
    *count = digit - '0';
    return true;
  }
...
_Use_decl_annotations_ bool hlsl::TryParseMatrixShorthand(
    const char *typeName, size_t typeNameLen, HLSLScalarType *parsedType,
    int *rowCount, int *colCount, const clang::LangOptions &langOptions) {
  //
...
hlsl::TryParseVectorShorthand(const char *typeName, size_t typeNameLen,
...
_Use_decl_annotations_ bool
hlsl::TryParseScalar(_In_count_(typenameLen) const char *typeName,
                     size_t typeNameLen, _Out_ HLSLScalarType *parsedType,
...
/// uint2) </summary>
_Use_decl_annotations_ bool
hlsl::TryParseAny(_In_count_(typenameLen) const char *typeName,
                  size_t typeNameLen, _Out_ HLSLScalarType *parsedType,
...
_Use_decl_annotations_ bool
hlsl::TryParseString(_In_count_(typenameLen) const char *typeName,
                     size_t typeNameLen,
                     _In_ const clang::LangOptions &langOptions) {
...
_Use_decl_annotations_ bool hlsl::TryParseMatrixOrVectorDimension(
    _In_count_(typeNameLen) const char *typeName, size_t typeNameLen,
    _Out_opt_ int *rowCount, _Out_opt_ int *colCount,
    _In_ const clang::LangOptions &langOptions) {
  *rowCount = 0;
  *colCount = 0;
