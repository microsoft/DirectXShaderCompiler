...
private:
  ...
  void PatchCreateHandleToUseDescriptorIndex(
      _In_ Module &M, _In_ IRBuilder<> &Builder,
      _In_ DXIL::ResourceKind &resourceKind,
      _In_ DXIL::ResourceClass &resourceClass, _In_ llvm::Type *resourceType,
...
bool DxilPatchShaderRecordBindings::GetHandleInfo(
    ...
    _Out_ unsigned int &shaderRegister, _Out_ unsigned int &registerSpace,
    _Out_ DXIL::ResourceKind &kind, _Out_ DXIL::ResourceClass &resClass,
    _Out_ llvm::Type *&resType) {
  DxilModule &DM = M.GetOrCreateDxilModule();
